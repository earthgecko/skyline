...
start () {
    ...
    if [ $USE_VIRTUALENV -eq 0 ]; then
      /usr/bin/env python "$BASEDIR/skyline/${SERVICE_NAME}/agent.py" start
    else
      $USE_PYTHON "$BASEDIR/skyline/${SERVICE_NAME}/agent.py" start
    fi
    RETVAL=$?
...
run () {
    ...
    if [ $USE_VIRTUALENV -eq 0 ]; then
      /usr/bin/env python "$BASEDIR/skyline/${SERVICE_NAME}/agent.py" run
    else
      $USE_PYTHON "$BASEDIR/skyline/${SERVICE_NAME}/agent.py" run
    fi
}
