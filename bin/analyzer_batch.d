...
# Catch instances where the process has not been stopped and there is no pid
# file
PROCESS_COUNT=$(ps aux | grep "${PARENT_SERVICE_NAME}/agent_batch.py start" | grep "$PARENT_SERVICE_NAME" | grep -v grep | awk '{print $2 }' | wc -l)
...
start () {
    ...
    if [ $USE_VIRTUALENV -eq 0 ]; then
      /usr/bin/env python "$BASEDIR/skyline/${PARENT_SERVICE_NAME}/agent_batch.py" start
    else
      $USE_PYTHON "$BASEDIR/skyline/${PARENT_SERVICE_NAME}/agent_batch.py" start
    fi
    RETVAL=$?
...
#      echo "error: A pid file exists for ${SERVICE_NAME} with no valid pid $PID_PATH/${SERVICE_NAME}.pid"
#      return 1
      echo "error: process/es exist for ${SERVICE_NAME} but there is no valid pid $PID_PATH/${SERVICE_NAME}.pid, stopping process/es"
    fi
...
#    fi
    ...
    SERVICE_RELATED_PID=$(ps aux | grep "${PARENT_SERVICE_NAME}/agent_batch.py start" | grep "$PARENT_SERVICE_NAME" | grep -v grep | awk '{print $2 }' | grep -c "$SERVICE_PID")
...
# Handle using a skyline user that does not have sudo access
  ...
  # TODO: write a real kill script
    ...
    if [ $PROCESS_COUNT -gt 0 ]; then
      # kill -15
      ps aux | grep "${PARENT_SERVICE_NAME}/agent_batch.py start" | grep "$PARENT_SERVICE_NAME" | grep -v grep | awk '{print $2 }' | while read i_pid
      do
        SERVICE_RELATED_PID=$(ps aux | grep "${PARENT_SERVICE_NAME}/agent_batch.py start" | grep "$PARENT_SERVICE_NAME" | grep -v grep | awk '{print $2 }' | grep -c "$i_pid")
        if [ $SERVICE_RELATED_PID -eq 1 ]; then
...
run () {
    ...
    if [ $USE_VIRTUALENV -eq 0 ]; then
      /usr/bin/env python "$BASEDIR/skyline/${PARENT_SERVICE_NAME}/agent_batch.py" run
    else
      $USE_PYTHON "$BASEDIR/skyline/${PARENT_SERVICE_NAME}/agent_batch.py" run
    fi
}
