...
def get_anomalies_for_period(
    ...
    if anomalies:
        try:
            ...
            current_logger.error('error :: query_anomalies.get_anomalies_for_period :: update_anomalies_with_names_for_ids failed - %s' % (
                err))
...
def get_anomaly(
    ...
    if anomaly:
        ...
        try:
            ...
            current_logger.error('error :: query_anomalies.get_anomaly :: update_anomalies_with_names_for_ids failed - %s' % (
                err))
        if anomalies:
...
def update_anomalies_with_names_for_ids(
    ...
    try:
        ...
        for anomaly_id in list(anomalies.keys()):
            ...
            if host_id and hosts_by_id:
                try:
                    ...
                    current_logger.error('error :: query_anomailes.update_anomalies_with_names_for_ids - failed to get host id from hosts for %s' % str(host_id))
            if host:
        ...
        current_logger.error(traceback.format_exc())
        current_logger.error('error :: update_anomalies_with_names_for_ids, err: %s' % err)
