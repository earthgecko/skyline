...
from custom_algorithms import significant_change_window_percent_sustained
...
def thunder_check_horizon_metrics_recieved(self):
    """
    ...
    if check_dict:
        try:
            ...
            timestamp = int(check_dict['timestamp'])
    ...
    if not check_thunder_alert:
        ...
        if alert_no_metrics_recieved_reported:
            ...
            status = '%s no count reported for 5 minutes' % metric
            thunder_event = {
                'level': level,
    ...
    if timeseries:
        ...
        times_in_a_row = 5
        try:
            times_in_a_row = int(settings.THUNDER_CHECKS[check_app][event_type]['times_in_a_row'])
        except Exception as e:
        ...
        try:
            anomalous, anomalyScore = significant_change_window_percent_sustained(skyline_app, getpid(), timeseries, algorithm_parameters)
        except Exception as e:
            logger.error('error :: %s :: failed to determine the expiry for %s %s check - %s' % (
                function_str, check_app, event_type, e))
        if anomalous:
    ...
    if check_thunder_alert and anomalous is False:
        ...
        status = '%s is now %.2f %% different from the median windows (%s seconds) of the last period (%s seconds)' % (
            metric, anomalyScore, str(window), str(period))
        thunder_event = {
            'level': level,
            'event_type': event_type,
            'message': message,
