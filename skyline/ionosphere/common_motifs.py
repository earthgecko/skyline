...
# @added 20260213 - Feature #5318: common_motifs
...
try:
    IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT = settings.IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT
except:
    IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT = {}

# @added 20260213 - Feature #5318: common_motifs
try:
    IONOSPHERE_AUTONOMOUS_LEARN_RATE = int(settings.IONOSPHERE_AUTONOMOUS_LEARN_RATE)
except:
    IONOSPHERE_AUTONOMOUS_LEARN_RATE = 43200
...
class MotifRemoval(Thread):
    """
    The MotifRemoval class which controls the snab thread and spawned
    threads.
    ...
    def spin_common_motifs(self, work_dict):
        """
        Assign motif removal work item to process.

        :param self: the self object
        :type self: object
        :param work_dict: a work dict from the
            ionosphere.find_repetitive_patterns.common_motifs.work Redis
            hash
        :type work_dict: dict
        :return: None
        :rtype: None
        ...
        """
        function_str = 'spin_common_motifs'

        logger.info('%s :: %s :: processing work' % (app_module, function_str))

        def remove_hash_key(
            hash_key, use_id, work_host, source_host,
            # @added 20250801 - Feature #5644: ionosphere.learn_self_validation
            # Allow for find_repetitive_patterns and learn_self_validation
            context='find_repetitive_patterns'):

            # @added 20250801 - Feature #5644: ionosphere.learn_self_validation
            redis_hash = 'ionosphere.%s.common_motifs.work' % context

            try:
                # @modified 20250801 - Feature #5644: ionosphere.learn_self_validation
                #removed = self.redis_conn_decoded.hdel('ionosphere.find_repetitive_patterns.common_motifs.work', hash_key)
                removed = self.redis_conn_decoded.hdel(redis_hash, hash_key)
                if removed:
                    logger.info('%s :: remove_hash_key :: removed %s from %s Redis hash' % (
                        app_module, redis_hash, hash_key))
            except Exception as err:
                logger.error('error :: %s :: remove_hash_key :: failed to remove %s from %s Redis hash, err: %s' % (
                    app_module, redis_hash, hash_key, err))
            if source_host == this_host:
                # @modified 20250801 - Feature #5644: ionosphere.learn_self_validation
                #done_key = 'ionosphere.find_repetitive_patterns.common_motifs.%s' % str(use_id)
                done_key = 'ionosphere.%s.common_motifs.%s' % (job, str(use_id))
            else:
                # @modified 20250801 - Feature #5644: ionosphere.learn_self_validation
                #done_key = 'ionosphere.find_repetitive_patterns.common_motifs.%s.%s' % (
                #    source_host, str(use_id))
                done_key = 'ionosphere.%s.common_motifs.%s.%s' % (
                    job, source_host, str(use_id))
            try:
        ...
        # @modified 20250806 - Feature #5644: ionosphere.learn_self_validation
        # Added context
        def candidate_timeseries(anomaly_timeseries, pw4_timeseries, context):
            """
            ...
            :type anomaly_timeseries: list
            :type pw4_timeseries: list
            :type context: str
            ...
            if max_found < 3:
                candidate = False
            # @added 20250806 - Feature #5644: ionosphere.learn_self_validation
            # Do not use similar minmax range as a condition for validation, similar
            # is a condition for learning, but not for validation.  Validation is
            # not concerned with ensuring all the motifs are within a similar enough
            # range to be considered or compared.  Similarity is required for the
            # learning context to ensure accuracy in learning.  This is not required
            # in validation.
            candidate_dict['context'] = context
            if context == 'learn_self_validation':
                candidate = True

            return candidate, candidate_dict

        # @modified 20250923 - Feature #5644: ionosphere.learn_self_validation 
        # Add hash_key to the function
        def remove_motifs(batch_size, max_distance, max_area_percent_diff, timeseries, hash_key):
            """
            ...
            :param hash_key: the work hash_key
            :type batch_size: int
            ...
            cache_used_for = []
            cached_errors = []
            motif_analysis_cache = {}
            motif_analysis_cache_str = None
            motif_analysis_cache_key = 'ionosphere.common_motifs.remove_motifs.%s' % str(hash_key)
            try:
                motif_analysis_cache_str = self.redis_conn_decoded.get(motif_analysis_cache_key)            
            except Exception as err:
                logger.error('error :: %s :: %s :: get failed on %s, err: %s' % (
                    app_module, function_str, str(hash_key), str(err)))
            if motif_analysis_cache_str:
                try:
                    motif_analysis_cache = literal_eval(motif_analysis_cache_str)
                except Exception as err:
                    if 'malformed node or string' in str(err):
                        if 'nan]' in motif_analysis_cache_str:
                            try:
                                motif_analysis_cache_str_no_nan = motif_analysis_cache_str.replace('nan]', 'None]')
                                motif_analysis_cache = literal_eval(motif_analysis_cache_str_no_nan)
                            except Exception as err2:
                                logger.error('error :: %s :: literal_eval failed on %s (with nan replacement str), err: %s' % (
                                    app_module, motif_analysis_cache_key, err2)) 
                    else:
                        logger.error('error :: %s :: %s :: literal_eval failed on %s data, err: %s' % (
                            app_module, function_str, str(motif_analysis_cache_key), str(err)))
            if motif_analysis_cache:
                try:
                    cached_indices_count = len(motif_analysis_cache[batch_size]['motifs'])
                    logger.info('%s :: %s :: %s cache key found with %s indices' % (
                        app_module, function_str, motif_analysis_cache_key, str(cached_indices_count)))
                except Exception as err:
                    logger.error('error :: %s :: %s :: failed to determine number of indices in cached motif_analysis, err: %s' % (
                        app_module, function_str, str(err)))
            # @added 20250925 -  - Feature #5644: ionosphere.learn_self_validation
            # Determine if there are sufficient candidate motifs based on
            # max_distance
            ...
            for batch_index in range(0, ((len(timeseries) - 1) - batch_size)):
                ...
                start_batch = time()
                start_step_time = float(start_batch)
                ...
                if not subsequences % 200:
                    logger.info('%s :: %s :: processed %s subsequences of %s total subsequences, motifs_found: %s, in %s seconds' % (
                        ...
                        str((time() - start_batch_size))))
                    logger.info('%s :: %s :: prefilter_filtered: %s, last operation timings: %s' % (
                        app_module, function_str, str(prefilter_filtered),
                        str(timings)))
                ...
                if batch_size in motif_analysis_cache.keys():
                    cached_result = None
                    try:
                        cached_result = motif_analysis_cache[batch_size]['motifs'][batch_index]
                    except Exception as err:
                        cached_errors.append(['cached_result', batch_index, err])
                    if cached_result:
                        try:
                            motif_analysis[batch_size]['motifs'][batch_index] = copy.deepcopy(cached_result)
                            cache_used_for.append(batch_index)
                            continue
                        except Exception as err:
                            cached_errors.append(['motif_analysis use cached_result', batch_index, err])
                ...
                timings = {}
                timings['check_data'] = time() - start_step_time
                ...
                timings['padding'] = time() - start_step_time
                start_step_time = time()
                ...
                candidate_indices = []
                if prefilter:
                    for idx in range(len(relate_dataset) - batch_size + 1):
                        window = relate_dataset[idx:idx + batch_size]
                        if min(window) >= min_y_padded and max(window) <= max_y_padded:
                            candidate_indices.append(idx)
                    timings['prefilter'] = time() - start_step_time
                    start_step_time = time()
                    # If fewer than 3 candidates, skip this subsequence entirely
                ...
                try:
                    ...
                    logger.error('error :: %s :: %s :: stumpy.match error: %s' % (
                        app_module, function_str, str(err)))
                    continue
                timings['match'] = time() - start_step_time
                start_step_time = time()
                ...
                timings['motifs_found'] = time() - start_step_time
                start_step_time = time()
                ...
                timings['motifs_found_in_indices'] = time() - start_step_time
                start_step_time = time()
                ...
                for motif in sorted_motifs:
                    try:
                        ...
                        relate_timeseries = timeseries[int(best_index):int(best_index + int(batch_size))]
                        ...
                        if not add_match:
                            ...
                            try:
                                ...
                                logger.error('error :: %s :: %s :: failed to get motif_area with np.trapezoid - %s' % (
                            ...
                            try:
                                ...
                                subsequence_motif_area = np.trapezoid(y_subsequence_motif, dx=1)
                            except Exception as err:
                                logger.error('error :: %s :: %s :: failed to get subseqence_motif_area with np.trapezoid - %s' % (
                                    app_module, function_str ,err))
                        ...
                        continue
                timings['motif_analysis'] = time() - start_step_time
                start_step_time = time()
                ...
                if len(motif_analysis[batch_size]['motifs'][batch_index].keys()) < 3:
                    ...
                    if make_cache:
                        try:
                            self.redis_conn_decoded.set(motif_analysis_cache_key, str(motif_analysis))
                            self.redis_conn_decoded.expire(motif_analysis_cache_key, 14400)
                        except Exception as err:
                            logger.error('error :: %s :: %s :: set failed on %s, err: %s' % (
                                app_module, function_str, str(motif_analysis_cache_key), str(err)))
                        timings['motif_analysis_cache'] = time() - start_step_time
                        start_step_time = time()

            logger.info('%s :: %s :: completed in %s seconds (cache used for %s indices)' % (
                app_module, function_str, (time() - start_all),
                str(len(cache_used_for))))

            return motif_analysis


        def motif_removed_timeseries_indices(
            motif_index, motif_timeseries, timeseries, known_removed_timeseries_indices=[], debug=False,
            use_predictions=True, max_distance=1.6):

            function_str = 'motif_removed_timeseries_indices'
            removed_timeseries_indices = set()
            motifs_matched = {}
            range_padding = 10
            ...
            if len(motif_timeseries) == 0:
                return removed_timeseries_indices, motifs_matched
            ...
            for motif in sorted_motifs:
                try:
                    ...
                    relate_timeseries = full_relate_timeseries[int(best_index):int(best_index + int(batch_size))]
                    relate_dataset = [item[1] for item in relate_timeseries]
                    ...
                    if add_match:
                        for removed_index in list(range(best_index, (best_index + batch_size))):
                            removed_timeseries_indices.add(removed_index)
            ...
            if debug:
                if len(removed_timeseries_indices) == 0:
                    logger.debug('debug :: %s :: %s :: motif_index: %s, batch_size: %s, %s, len(match_results_list): %s, motifs removed: %s data points, new_removals: 0, completed in %s seconds' % (
                        app_module, function_str, str(motif_index), str(batch_size),
                        start_index_datestr, str(len(match_results_list)),
                        str(len(removed_timeseries_indices)), (time() - start_all)))
                else:
                    new_removals = [a_i for a_i in removed_timeseries_indices if a_i not in known_removed_timeseries_indices]
                    logger.debug('debug :: %s :: %s :: motif_index: %s, batch_size: %s, %s, len(match_results_list): %s, motifs removed: %s data points, new_removals: %s, completed in %s seconds' % (
                        app_module, function_str, str(motif_index), str(batch_size),
                        start_index_datestr, str(len(match_results_list)),
                        str(len(removed_timeseries_indices)),
                        len(new_removals), (time() - start_all)))

            return removed_timeseries_indices, motifs_matched


        def remove_timeseries(batch_size, anomaly_timeseries, removed_timeseries_indices, pw5_timeseries):
            """
            Given a batch_size, an anomly_timeseries,
            the removed_timeseries_indices and a pw5_timeseries remove
            ...
            motif is removed from the anomaly_timeseries.  The
            ...
            :param removed_timeseries_indices: the max_area_percent_diff under which a motif is
                considered similar
            ...
            """
            function_str = 'remove_timeseries'

            removed_anomaly_timeseries = []
            removed_count = 0
            unremoved_anomaly_timeseries = []
            for index, item in enumerate(anomaly_timeseries):
                t = item[0]
                v = item[1]
                if index in removed_timeseries_indices:
                    v = -1
                    removed_count += 1
                else:
                    unremoved_anomaly_timeseries.append([t, v])
                removed_anomaly_timeseries.append([t, v])
            unremoved_dict = {}
            removed_unremoved_dict = {}

            # Handle if there are multiple data points not removed
            # and they are all the same value, normally 0
            same_value = []
            if unremoved_anomaly_timeseries:
                same_value = list(set([v for t, v in unremoved_anomaly_timeseries]))

            # If the remaining unremoved motifs (even if a single data point) are
            # less than batch_size consider for micro motif removal
            max_item_size = 0
            # Determine the max size of remaining unremoved motifs (even if a
            # single data point)
            last_unremoved_timestamp = None
            last_unremoved_motif = []
            for item in unremoved_anomaly_timeseries:
                if not last_unremoved_timestamp:
                    last_unremoved_timestamp = item[0]
                    last_unremoved_motif.append(item)
                    continue
                if not last_unremoved_motif:
                    last_unremoved_motif = [item]
                if item[0] > (last_unremoved_timestamp + 600):
                    if last_unremoved_motif:
                        last_unremoved_motif_size = len(last_unremoved_motif)
                        if last_unremoved_motif_size > max_item_size:
                            max_item_size = int(last_unremoved_motif_size)
                    last_unremoved_motif = []
                else:
                    last_unremoved_motif.append(item)
                last_unremoved_timestamp = item[0]
            ...
            # If the timeseries was not removed and less than batch_size
            # indices remain then attempt to remove each index based on
            ...
            # range too.
            if removed_count < len(anomaly_timeseries) or len(same_value) == 1 or max_item_size < (batch_size * 2):
                unremoved_index_count = len(anomaly_timeseries) - removed_count
                logger.info('%s :: %s :: the anomaly timeseries was not removed by common motifs and %s indices remain' % (
                    app_module, function_str, str(unremoved_index_count)))

                if removed_count >= (len(anomaly_timeseries) - (batch_size - 1)) or len(same_value) == 1 or max_item_size < (batch_size * 2):
                    for index, item in enumerate(anomaly_timeseries):
                        if index not in removed_timeseries_indices:
                            pw5_index = None
                            delta = 0
                            if index != 0:
                            ...
                            unremoved_dict[index] = {'timestamp': item[0], 'value': item[1], 'delta': delta}
                            if pw5_index:
                                # These are used to calculate the delta from
                                unremoved_dict[index]['pw5_index'] = pw5_index - 1
                                unremoved_dict[index]['pw5_value'] = pw5_value
            removed_unremoved_count = 0
            removed_unremoved_dict = {}

            # @added 20250826 - Feature #5644: ionosphere.learn_self_validation
            remove_micro_motif_keys = []

            # micro motif removal
            for key, data in unremoved_dict.items():
                removed_unremoved_dict[key] = data
                removed_unremoved_dict[key]['removed'] = False
                value = data['value']
                range_padding = 10
                if value == 0:
                    min_y = float(value)
                ...
                removed_unremoved_dict[key]['min_y'] = min_y
                removed_unremoved_dict[key]['max_y'] = max_y
                found_values = [[index, item] for index, item in enumerate(pw5_timeseries) if item[1] >= min_y and item[1] <= max_y]
                removed_unremoved_dict[key]['value_found_count'] = len(found_values)
                ...
                removed_unremoved_dict[key]['min_delta'] = min_delta
                removed_unremoved_dict[key]['max_delta'] = max_delta
                if min_delta < 0 and max_delta < 0:
                    # @modified 20250826 - Feature #5644: ionosphere.learn_self_validation
                    ...
                    found_deltas = [delta for delta in delta_values if delta >= min_delta and delta <= max_delta]
                removed_unremoved_dict[key]['delta_found_count'] = len(found_deltas)
                # If at least 4 similar values and at least 4 similar deltas were found
                # class the micro motif as common and remove the data point
                if len(found_deltas) >= 4:
                    match_index = found_values[0][0]
                    removed_timeseries_indices.append(match_index)
                    removed_anomaly_timeseries[key] = [data['timestamp'], -1]
                    removed_count += 1
                    removed_unremoved_count += 1
                    removed_unremoved_dict[key]['removed'] = True
                    # @added 20250826 - Feature #5644: ionosphere.learn_self_validation
                    remove_micro_motif_keys.append(key)

            # @added 20250826 - Feature #5644: ionosphere.learn_self_validation
            for key in remove_micro_motif_keys:
                logger.info('%s :: %s :: removing micro motif at index %s' % (
                    app_module, function_str, str(key)))
                try:
                    del removed_unremoved_dict[key]
                except Exception as err:
                    logger.info('%s :: %s :: failed to remove %s from removed_unremoved_dict, err: %s' % (
                        app_module, function_str, str(key), err))
                try:
                    del unremoved_dict[key]
                except Exception as err:
                    logger.info('%s :: %s :: failed to remove %s from unremoved_dict, err: %s' % (
                        app_module, function_str, str(key), err))

            if len(unremoved_dict) > 0:
                logger.info('%s :: %s :: removed_unremoved_dict: %s' % (
                    app_module, function_str, str(removed_unremoved_dict)))
                logger.info('%s :: %s :: mirco motifs removed: %s of the remaining %s unremoved indices' % (
                    app_module, function_str, str(removed_unremoved_count),
                    str(len(unremoved_dict))))
            logger.info('%s :: %s :: removed: %s indices of the %s indices in the anomaly_timeseries' % (
                app_module, function_str, str(removed_count), str(len(anomaly_timeseries))))
            return removed_anomaly_timeseries, removed_count, unremoved_dict, removed_unremoved_dict
        ...
        def plot_match(
            ...
            try:
                ...
                context = 'common_motifs - ANNIHILATED anomalous time series\n%s' % metric
                ...
                if pw5_timeseries:
                    not_anomalous_label = 'removed anomaly time series'
                    not_anomalous_line_color = 'green'
                ...
                matched_label = 'removed anomalous pattern'
                if pw5_timeseries:
        ...
        def plot_validation(metric, fp_id, pw4_timeseries, fp_timeseries, output_file):
            try:
                matplotlib.rcParams['path.simplify_threshold'] = 1.0
                subsequence_values = [item[1] for item in pw4_timeseries]
                fp_timestamps = [item[0] for item in fp_timeseries]
                # Plot validation
                rcParams['figure.figsize'] = 10, 4
                # Improve matplotlib render performance
                rcParams['path.simplify_threshold'] = 1.0
                ...
                fig = plt.figure(frameon=False)
                ax = fig.add_subplot(111)
                context = 'common_motifs - validated LEARNT features profile - %s\n%s' % (str(fp_id), metric)                    
                try:
                    match_subsequence_values = []
                    for ts, value in pw4_timeseries:
                        if ts not in fp_timestamps:
                            value = None
                        match_subsequence_values.append(value)
                except Exception as err:
                    logger.error('error :: %s :: plot_validation :: match_subsequence_values, err: %s' % (
                        app_module, err))
                    return False
                ...
                if hasattr(ax, 'set_facecolor'):
                    ...
                    ax.set_axis_bgcolor('white')
                datetimes = [datetime.fromtimestamp(int(item[0]), timezone.utc) for item in pw4_timeseries]
                ...
                fp_label = 'validated features profile'
                fp_line_color = 'green'

                ax.plot(
                    datetimes, match_subsequence_values, label=fp_label,
                    color=fp_line_color, lw=1, linestyle='solid',
                    zorder=4)
                ax.tick_params(axis='both', labelsize='small')
                matched_label = 'recent 4 weeks of features profile period'
                ...
                ax_text = 'data points: %s' % str(len(subsequence_values))
                ax_text_color = 'green'
                ax_text_color = fp_line_color
                ax.text(text_x, text_y, ax_text, size=8, color=ax_text_color, alpha=0.7)
                plt.savefig(output_file, format='png')
                fig.clf()
                plt.close(fig)
                logger.info('%s :: plot_validation :: plotted %s' % (
                    app_module, output_file))
                return output_file
            except Exception as err:
                logger.error('error :: %s :: plot_validation :: plot failed, err: %s' % (
                    app_module, err))
                return False
        ...
        def plot_removal(
            ...
            try:
                ...
                logger.error('error :: %s :: %s :: plot_match failed to plot match, err: %s' % (
                    app_module, function_str, err))
            try:
                output_file = output_file.replace('common_motifs', 'common_motifs.derived')
            ...
            try:
                output_file = output_file.replace('common_motifs.derived', 'common_motifs.pw5_timeseries')
        ...
        def determine_external_analysis_host(metric, external_common_motifs_hosts):
            try:
                external_hosts = list(external_common_motifs_hosts.keys())
        ...
        metric = None
        results = {}
        anomaly_timeseries_removed = False
        source_host = str(this_host)
        work_host = str(this_host)
        # @added 20250801 - Feature #5644: ionosphere.learn_self_validation
        learn_self_validation = False
        fp_id = None
        job = 'find_repetitive_patterns'
        context = 'find_repetitive_patterns'
        ...
        try:
            ...
            logger.error('error :: %s :: %s :: failed to determine details from work_dict, err: %s' % (
                app_module, function_str, err))
            remove_hash_key(hash_key, 0, work_host, source_host, context=context)
            return

        # @added 20250801 - Feature #5644: ionosphere.learn_self_validation
        common_motifs_done_key = 'ionosphere.common_motifs.learn_self_validation.work_done'
        use_id = int(anomaly_id)
        try:
            context = work_dict['context']
            if context == 'learn_self_validation':
                learn_self_validation = True
            fp_id = work_dict['fp_id']
            use_id = int(fp_id)
        except:
            learn_self_validation = False
            fp_id = None
        learn_self_validation_adhoc = False
        if learn_self_validation:
            try:
                learn_self_validation_adhoc = work_dict['adhoc']
            except:
                learn_self_validation_adhoc = False

        logger.info('%s :: %s :: processing %s for %s' % (
            app_module, function_str, str(hash_key), context))
        ...
        if source_host == this_host:
            ...
            done_key = 'ionosphere.%s.common_motifs.%s' % (context, str(use_id))
        else:
            # @modified 20250801 - Feature #5644: ionosphere.learn_self_validation
            #done_key = 'ionosphere.find_repetitive_patterns.common_motifs.%s.%s' % (
            #    source_host, str(anomaly_id))
            done_key = 'ionosphere.%s.common_motifs.%s.%s' % (
                context, source_host, str(use_id))
        ...
        if not results_url and not external_results:
            ...
            try:
                ...
                logger.error('error :: %s :: %s :: exists failed on %s Redis key, err: %s' % (
                    app_module, function_str, done_key, err))
            if learn_self_validation_adhoc:
                # Rerun even if the done key exists
                done_key_exists = False
            if done_key_exists:
                logger.info('%s :: %s :: work already done for id: %s, results in removing nothing to do' % (
                    app_module, function_str, str(use_id)))
                done_key_data = {}
                try:
                    done_key_data = self.redis_conn_decoded.get(done_key)
                except Exception as err:
                    logger.error('error :: %s :: %s :: get failed on %s Redis key, err: %s' % (
                        app_module, function_str, done_key, err))
                try:
                    self.redis_conn_decoded.hset(common_motifs_done_key, fp_id, str(done_key_data))
                except Exception as err:
                    logger.error('error :: %s :: %s :: hset failed to add %s, err: %s' % (
                        app_module, function_str, str(fp_id), err))            
                remove_hash_key(hash_key, use_id, work_host, source_host, context=context)
                return
        ...
        if local_metric:
            ...
            if not os.path.exists(metric_data_dir):
                logger.info('%s :: %s :: nothing to do, no metric_data_dir: %s' % (
                    app_module, function_str, metric_data_dir))
                remove_hash_key(hash_key, use_id, work_host, source_host, context=context)
                return
        ...
        # @added 20250801 - Feature #5644: ionosphere.learn_self_validation
        # With a learn_self_validation job the shuffled validation time series
        # is passed as pw5_timeseries.
        # NOTE:
        # In the context of a learn_self_validation job, the method is to simply
        # validate that the features profile which was learnt is valid when
        # considered in the context of 1 week before the learnt features profile
        # and 2 weeks after the learnt features profile.  Therefore this
        # pw5_timeseries is actually a preprocessed 4 week time series, where
        # the most current 2 weeks are timeshifted:
        ...
        if learn_self_validation:
            try:
                ...
                logger.error('error :: %s :: %s :: failed to determine pw5_timeseries from learn_self_validation work_dict, err: %s' % (
                    app_module, function_str, err))
                remove_hash_key(hash_key, use_id, work_host, source_host, context=context)
                return
        ...
        if not pw5_timeseries and not external_results:
            ...
            # Replace nan with None from VictoriaMetrics data
            if '_tenant_id="' in metric and pw5_timeseries:
                try:
                    pw5_timeseries = [[ts, None if np.isnan(v) else v] for ts, v in pw5_timeseries]
                except Exception as err:
                    logger.error('error :: %s :: %s :: failed to replace nan with None in VictoriaMetrics pw5_timeseries, err: %s' % (
                        app_module, function_str, err))
        ...
        if not pw5_timeseries:
            logger.info('%s :: %s :: no pw5_timeseries was surfaced, nothing to do for %s' % (
                app_module, function_str, context))
            remove_hash_key(hash_key, use_id, work_host, source_host, context=context)
            return

        # Outsource the analysis to an external Skyline node
        external_analysis_host = None
        if IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT and not results_url:
            # Map the metric to a host
            try:
                external_analysis_host = determine_external_analysis_host(metric, IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT)
            except Exception as err:
        ...
        if external_analysis_host:
            ...
            # @added 20250801 - Feature #5644: ionosphere.learn_self_validation
            redis_work_hash = 'ionosphere.%s.common_motifs.work' % context

            try:
                work_dict['source_host'] = this_host
                work_dict['app'] = skyline_app
                work_dict['job'] = 'common_motifs'
                # @modified 20250801 - Feature #5644: ionosphere.learn_self_validation
                #work_dict['redis_work_hash'] = 'ionosphere.find_repetitive_patterns.common_motifs.work'
                work_dict['redis_work_hash'] = redis_work_hash
                ...
                # @added 20250801 - Feature #5644: ionosphere.learn_self_validation
                work_dict['context'] = context

                work_url = '%s/wind' % IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT[external_analysis_host]['url']
                work_dict['work_url'] = work_url
                if 'auth_user' in IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT[external_analysis_host].keys():
                    work_dict['auth_user'] = IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT[external_analysis_host]['auth_user']
                if 'auth_password' in IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT[external_analysis_host].keys():
                    work_dict['auth_password'] = IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT[external_analysis_host]['auth_password']
                if user:
            ...
            # @added 20250923 - Feature #5644: ionosphere.learn_self_validation 
            # Limit how many items can be added to the work queue, if a 200 is
            # returned the work item was added, if a 204 is returned the queue
            # is full
            status_code = 0
            if submitted_job_response_dict:
                try:
                    status_code = submitted_job_response_dict['status_code']
                except Exception as err:
                    logger.error('error :: %s :: %s :: failed to determine status_code from submitted_job_response_dict, err: %s' % (
                        app_module, function_str, err))
                    status_code = 0

            if submitted_job_response_dict:
                logger.info('%s :: %s :: submitted job to external_analysis_host and got status_code: %s' % (
                    app_module, function_str, str(status_code)))

            # @modified 20250923 - Feature #5644: ionosphere.learn_self_validation 
            # Limit how many items can be added to the work queue, if a 200 is
            # returned the work item was added, if a 204 is returned the queue
            # is full.  Only remove the key if the work was added
            #remove_hash_key(hash_key, use_id, work_host, source_host, context=context)
            if str(status_code) == '200':

                # @added 20250925 - Feature #5644: ionosphere.learn_self_validation 
                # Set a key that the work was submitted to an external host
                # which can be checked if there is any mismatches that occur
                # between this host and the external host
                wind_hash_key = 'ionosphere.common_motifs.wind.hash_key.%s' % str(hash_key)
                try:
                    self.redis_conn_decoded.setex(wind_hash_key, 7200, str([external_analysis_host, 7200, int(time())]))
                except Exception as err:
                    logger.error('error :: %s :: %s :: setex failed on %s, err: %s' % (
                        app_module, function_str, wind_hash_key, err))            
                wind_fp_id_hash_key = 'ionosphere.common_motifs.wind.fp_id.%s' % str(fp_id)
                try:
                    self.redis_conn_decoded.setex(wind_fp_id_hash_key, 7200, str([external_analysis_host, 7200, int(time())]))
                except Exception as err:
                    logger.error('error :: %s :: %s :: setex failed on %s, err: %s' % (
                        app_module, function_str, wind_fp_id_hash_key, err))            

                remove_hash_key(hash_key, use_id, work_host, source_host, context=context)
            else:
                logger.info('%s :: %s :: external_analysis_host did not accept the work item got status_code: %s' % (
                    app_module, function_str, str(status_code)))
            if str(status_code) == '204':
                try:
                    self.redis_conn_decoded.setex('ionosphere.common_motifs.full_queue', 60, 1)
                    logger.info('%s :: %s :: set ionosphere.common_motifs.full_queue' % (app_module, function_str))
                except Exception as err:
                    logger.error('error :: setex failed on ionosphere.common_motifs.full_queue, err: %s' % err)
        ...
        if results_url:
            ...
            results['anomaly_timeseries_removed'] = anomaly_timeseries_removed
            results['learn'] = learn
        ...
        if not external_results:
            ...
            candidate, candidate_dict = candidate_timeseries(anomaly_timeseries, pw4_timeseries, context)
        ...
        if not candidate:
            logger.info('%s :: %s :: not checking for removal motifs because the min or max values were not found at least 4 times in the pw4_timeseries' % (
                app_module, function_str))
            if results_url:
                logger.info('%s :: %s :: returning wind results' % (
                    app_module, function_str))
                try:
            ...
            # @added 20250804 - Feature #5644: ionosphere.learn_self_validation
            if learn_self_validation:
                logger.info('%s :: %s :: %s updating %s' % (
                    app_module, function_str, context, common_motifs_done_key))
                results['learn'] = learn
                work_dict['learn'] = learn
                work_dict['results'] = copy.deepcopy(results)
                try:
                    self.redis_conn_decoded.hset(common_motifs_done_key, fp_id, str(work_dict))
                except Exception as err:
                    logger.error('error :: %s :: %s :: hset failed to add %s, err: %s' % (
                        app_module, function_str, str(fp_id), err))            
                try:
                    work_dict_to_use = copy.deepcopy(work_dict)
                    if not results_url:
                        del work_dict_to_use['pw5_timeseries']
                        del work_dict['pw5_timeseries']
                    self.redis_conn_decoded.setex(done_key, (86400), str(work_dict_to_use))
                except Exception as err:
                    logger.error('error :: %s :: %s :: setex failed to add key %s, err: %s' % (
                        app_module, function_str, done_key, err))
                logger.info('%s :: %s :: common motifs, validated %s' % (
                    app_module, function_str, str(learn)))
        ...
        if external_results:
            try:
                ...
                logger.info('%s :: %s :: %s removal motifs found in external_results' % (
        ...
        if not external_results and not motif_analysis:
            try:
                # @modified 20250923 - Feature #5644: ionosphere.learn_self_validation 
                # Add hash_key to the function
                #motif_analysis = remove_motifs(batch_size, max_distance, max_area_percent_diff, pw4_timeseries)
                motif_analysis = remove_motifs(batch_size, max_distance, max_area_percent_diff, pw4_timeseries, hash_key)
            except Exception as err:
                logger.error('error :: %s :: %s :: remove_motifs failed, err: %s' % (
                    app_module, function_str, err))
        if results_url:
        ...
        logger.info('%s :: %s :: %s removal motifs found' % (
            app_module, function_str, str(len(list(motif_analysis[batch_size]['motifs'].keys())))))

        removed_timeseries_indices = []
        matched_motifs = {}

        if external_results:
            try:
                removed_timeseries_indices = external_results['removed_timeseries_indices']
                logger.info('%s :: %s :: %s removed_timeseries_indices found in external_results' % (
                    app_module, function_str, str(len(removed_timeseries_indices))))
            except Exception as err:
                logger.error('error :: %s :: %s :: failed to get removed_timeseries_indices from external_results, err: %s' % (
                    app_module, function_str, err))
            try:
        ...
        if not external_results:
            ...
            start_removal = time()
            logger.info('%s :: %s :: starting motif_removed_timeseries_indices, %s motifs to check' % (
                app_module, function_str, str(len(list(motif_analysis[batch_size]['motifs'].keys())))))
            anomaly_timeseries_indices = [index for index, item in enumerate(anomaly_timeseries)]
            for motif_id in list(motif_analysis[batch_size]['motifs'].keys()):
                ...
                if not done % 200:
                    logger.info('%s :: %s :: %s motif_removed_timeseries_indices checked' % (
                        app_module, function_str, str(done)))

                motif_timeseries = pw4_timeseries[motif_id:(motif_id + batch_size)]
                motif_removed_timeseries_indices_set = []
                try:
                    motif_removed_timeseries_indices_set, motifs_matched = motif_removed_timeseries_indices(motif_id, motif_timeseries, anomaly_timeseries_plus_3600, known_removed_timeseries_indices=removed_timeseries_indices, debug=False)
                    matched_motifs[motif_id] = motifs_matched
                except Exception as err:
                    logger.error(traceback.format_exc())
                    logger.error('error :: %s :: %s :: motif_removed_timeseries_indices failed on motif_id: %s, err: %s' % (
                        app_module, function_str, str(motif_id), err))
                    continue
                for i in list(motif_removed_timeseries_indices_set):
                    removed_timeseries_indices.append(i)
                removed_timeseries_indices = sorted(list(set(removed_timeseries_indices)))
                if removed_timeseries_indices == anomaly_timeseries_indices:
                    logger.info('%s :: %s :: timeseries removed after %s motifs compared of %s motifs' % (
                        app_module, function_str, str(done),
                        str(len(list(motif_analysis[batch_size]['motifs'].keys())))))
                    break
            logger.info('%s :: %s :: removed %s indices in %s seconds' % (
                app_module, function_str, str(len(removed_timeseries_indices)),
                str(time() - start_removal)))

            # @added 20250923 - Feature #5644: ionosphere.learn_self_validation 
            # Add hash_key to the function
            motif_analysis_cache_key = 'ionosphere.common_motifs.remove_motifs.%s' % str(hash_key)
            try:
                self.redis_conn_decoded.delete(motif_analysis_cache_key, str(motif_analysis_cache_key))
            except Exception as err:
                logger.error('error :: %s :: %s :: set failed on %s, err: %s' % (
                    app_module, function_str, str(motif_analysis_cache_key), str(err)))

            if results_url:
                results['removed_timeseries_indices'] = removed_timeseries_indices
        ...
        if external_results:
            ...
            try:
                removed_anomaly_timeseries = external_results['removed_anomaly_timeseries']
                logger.info('%s :: %s :: %s removed_anomaly_timeseries found in external_results' % (
                    app_module, function_str, str(len(removed_anomaly_timeseries))))
            except Exception as err:
                logger.error('error :: %s :: %s :: failed to get removed_timeseries_indices from external_results, err: %s' % (
                    app_module, function_str, err))
                external_results_error = True
            try:
                removed_count = external_results['removed_count']
                logger.info('%s :: %s :: removed_count: %s, found in external_results' % (
                    app_module, function_str, str(removed_count)))
            except Exception as err:
                logger.error('error :: %s :: %s :: failed to get removed_count from external_results, err: %s' % (
                    app_module, function_str, err))
                external_results_error = True
            try:
                unremoved_dict = external_results['unremoved_dict']
                logger.info('%s :: %s :: %s items in unremoved_dict found in external_results' % (
                    app_module, function_str, str(len(unremoved_dict))))
            except Exception as err:
                logger.error('error :: %s :: %s :: failed to get unremoved_dict from external_results, err: %s' % (
                    app_module, function_str, err))
                external_results_error = True
            try:
                removed_unremoved_dict = external_results['removed_unremoved_dict']
                logger.info('%s :: %s :: %s items removed_removed_unremoved_dict found in external_results' % (
                    app_module, function_str, str(len(removed_unremoved_dict))))
            except Exception as err:
                logger.error('error :: %s :: %s :: failed to get removed_unremoved_dict from external_results, err: %s' % (
                    app_module, function_str, err))
                external_results_error = True
        ...
        if not external_results:
            logger.info('%s :: %s :: running remove_timeseries for %s' % (
                app_module, function_str, context))
            try:
                ...
                removed_anomaly_timeseries, removed_count, unremoved_dict, removed_unremoved_dict = remove_timeseries(batch_size, anomaly_timeseries, removed_timeseries_indices, pw5_timeseries)
            except Exception as err:
                logger.error(traceback.format_exc())
                logger.error('error :: %s :: %s :: remove_timeseries failed, err: %s' % (
                    app_module, function_str, err))
            # If there are micro motif removals add them to the matched_motifs
            if removed_unremoved_dict and matched_motifs:
                micro_motifs_key = list(matched_motifs.keys())[-1] + batch_size
                matched_motifs[micro_motifs_key] = {}
                for key, item in removed_unremoved_dict.items():
                    data = {
            ...
            if results_url:
                results['matched_motifs'] = dict(matched_motifs)
                results['removed_anomaly_timeseries'] = removed_anomaly_timeseries
                results['removed_count'] = removed_count
                results['unremoved_dict'] = unremoved_dict
                results['removed_unremoved_dict'] = removed_unremoved_dict
        if removed_count == len(anomaly_timeseries):
            learn = True
            anomaly_timeseries_removed = True
            logger.info('%s :: %s :: REMOVED - the anomaly timeseries was removed by common motifs, good to LEARN for %s' % (
                app_module, function_str, context))
            if results_url:
                results['learn'] = learn
                results['anomaly_timeseries_removed'] = anomaly_timeseries_removed

        removal_process_runtime = time() - start_all
        logger.info('%s :: %s :: the removal process took %s seconds' % (
            app_module, function_str, str(removal_process_runtime)))
        ...
        if learn_self_validation:
            ...
            fp_timeseries = None
            if learn:
                if 'pw4_timeseries' in work_dict.keys():
                    logger.info('%s :: %s :: pw4_timeseries available for plotting validation plot' % (
                        app_module, function_str))
                    try:
                        pw4_timeseries = work_dict['pw4_timeseries']
                    except Exception as err:
                        logger.error('error :: %s :: %s :: pw4_timeseries failed, err: %s' % (
                            app_module, function_str, err))
                if pw4_timeseries and 'fp_timeseries' in work_dict.keys():
                    logger.info('%s :: %s :: fp_timeseries available for plotting validation plot' % (
                        app_module, function_str))
                    try:
                        fp_timeseries = work_dict['fp_timeseries']
                        plot_data = True
                    except Exception as err:
                        logger.error('error :: %s :: %s :: fp_timeseries failed, err: %s' % (
                            app_module, function_str, err))

            # @added 20250923 - Feature #5644: ionosphere.learn_self_validation
            if 'source_host' in work_dict:
                try:
                    if work_dict['source_host'] != this_host:
                        plot_data = False
                except Exception as err:
                    logger.error('error :: %s :: %s :: failed to compare soure_host for plot, err: %s' % (
                        app_module, function_str, err))
            ...
            if plot_data:
                try:
                    ...
                    metric_data_dir = '%s/%s/%s' % (
                        settings.IONOSPHERE_PROFILES_FOLDER, metric_timeseries_dir,
                        work_dict['anomaly_timestamp'])
                    validated_plot_file = '%s/%s.fp-id.%s.common_motifs.validated.png' % (
                        metric_data_dir, use_metric, str(fp_id))
                    logger.info('%s :: %s :: plotting validation plot - %s' % (
                        app_module, function_str, validated_plot_file))
                    validation_plotted_file = plot_validation(
                            metric, fp_id, pw4_timeseries, fp_timeseries, 
                            validated_plot_file)
                except Exception as err:
                    logger.error('error :: %s :: %s :: validation_plotted_file failed, err: %s' % (
                        app_module, function_str, err))
            ...
            try:
                ...
                logger.error('error :: %s :: %s :: hset failed to add %s, err: %s' % (
                    app_module, function_str, str(fp_id), err))            
            try:
                # @modified 20250818 - Feature #5644: ionosphere.learn_self_validation
                # Removing the pw5_timeseries from the work_dict now causes the
                # return_wind_results to fail as webapp reports it is not a
                # valid wind request as there is no pw5_timeseries.  This after
                # client_max_body_size 30M was set in the nginx config to allow
                # for the pw5 and pw4 timeseries and results to be posted
                #del work_dict['pw5_timeseries']
                #self.redis_conn_decoded.setex(done_key, (86400), str(work_dict))
                cache_work_dict = copy.deepcopy(work_dict)
                del cache_work_dict['pw5_timeseries']
                self.redis_conn_decoded.setex(done_key, (86400), str(cache_work_dict))
                del cache_work_dict
            except Exception as err:
                logger.error('error :: %s :: %s :: setex failed to add key %s, err: %s' % (
                    app_module, function_str, done_key, err))
        ...
        if results_url:
            results['removal_process_runtime'] = removal_process_runtime            
            try:
                return_wind_results(results_url, work_dict, results)
            except Exception as err:
                logger.error('error :: %s :: %s :: return_wind_results failed, err: %s' % (
        ...
        if not learn:
            logger.info('%s :: %s :: the anomaly timeseries could not be removed by common motifs, not valid to LEARN' % (
                app_module, function_str))
            remove_hash_key(hash_key, use_id, work_host, source_host, context=context)
            return
        ...
        pw5_json_file = '%s/%s.common_motifs.pw5_timeseries.json' % (metric_data_dir, use_metric)
        ...
        matched_motifs_file = '%s/%s.common_motifs.matched_motifs.json' % (metric_data_dir, use_metric)
        ...
        removal_plot_file = '%s/%s.common_motifs.png' % (metric_data_dir, use_metric)
        plotted_files = []
        try:
            plotted_files = plot_removal(use_metric, batch_size, original_anomaly_timeseries, matched_motifs, original_pw4_timeseries, removal_plot_file)
            logger.info('%s :: %s :: plot_removal created %s plots' % (
                app_module, function_str, str(len(plotted_files))))
        except Exception as err:
            logger.error('error :: %s :: %s :: plot_removal failed to add %s, err: %s' % (
                app_module, function_str, removal_plot_file, err))
        if plotted_files:
        ...
        features_file = '%s/%s.tsfresh.input.csv.features.transposed.csv' % (metric_data_dir, use_metric)
        ...
        try:
            ...
            slack_ionosphere_job = 'common_motifs'
            user_id = 1
            label = 'LEARNT - common_motifs'
        ...
        if external_results:
            external_done_key = 'ionosphere.find_repetitive_patterns.common_motifs.%s' % str(anomaly_id)
            external_done_key_exists = False
            try:
                external_done_key_exists = self.redis_conn_decoded.exists(external_done_key)
            except Exception as err:
                logger.error('error :: %s :: remove_hash_key :: exists failed on %s Redis key, err: %s' % (
                    app_module, external_done_key, err))
            if external_done_key_exists:
                logger.info('%s :: %s :: work already done for id: %s, removing nothing to do' % (
                    app_module, function_str, str(use_id)))
                remove_hash_key(hash_key, use_id, work_host, source_host, context=context)
                return
    ...
    def run(self):
        ...
        logger.info('starting %s common_motifs' % skyline_app)
        if os.path.isfile(skyline_app_loglock):
        ...
        while True:
            ...
            while True:
                ...
                full_queue = False
                try:
                    full_queue = self.redis_conn_decoded.exists('ionosphere.common_motifs.full_queue')
                except Exception as err:
                    logger.error('error :: could not query Redis ionosphere.common_motifs.full_queue, err: %s' % err)
                if full_queue:
                    logger.info('%s :: ionosphere.common_motifs.full_queue exists, sleeping for 60 seconds' % app_module)
                    sleep(59)
                    if IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT:
                        continue
                ...
                common_motifs_work = {}
                find_repetitive_patterns_common_motifs_work_count = 0
                try:
                    find_repetitive_patterns_common_motifs_work_count = self.redis_conn_decoded.hlen('ionosphere.find_repetitive_patterns.common_motifs.work')
                except Exception as err:
                    logger.error('error :: hlen falied on ionosphere.find_repetitive_patterns.common_motifs.work, err: %s' % err)
                if find_repetitive_patterns_common_motifs_work_count:
                    try:
                        common_motifs_work = self.redis_conn_decoded.hgetall('ionosphere.find_repetitive_patterns.common_motifs.work')
                    except Exception as err:
                        logger.error('error :: hgetall failed on ionosphere.find_repetitive_patterns.common_motifs.work, err: %s' % err)

                # @added 20250801 - Feature #5644: ionosphere.learn_self_validation
                context_hash_key = 'ionosphere.find_repetitive_patterns.common_motifs.work'
                common_motifs_validation_work = {}
                if len(common_motifs_work) == 0:
                    try:
                        common_motifs_validation_work = self.redis_conn_decoded.hgetall('ionosphere.learn_self_validation.common_motifs.work')
                    except Exception as err:
                        logger.error('error :: could not query Redis hash ionosphere.learn_self_validation.common_motifs.work, err: %s' % err)
                    if common_motifs_validation_work:
                        common_motifs_work = copy.deepcopy(common_motifs_validation_work)
                        context_hash_key = 'ionosphere.learn_self_validation.common_motifs.work'

                # @modified 20250804 - Feature #5644: ionosphere.learn_self_validation
                #if len(common_motifs_work) == 0:
                if len(common_motifs_work) == 0 and len(common_motifs_validation_work):
                    logger.info('%s :: no work, sleeping for 60 seconds' % app_module)
                    sleep(59)
                    continue

                skyline_busy = False
                if not IONOSPHERE_EXTERNAL_COMMON_MOTIFS_HOSTS_DICT:
                    try:
                ...
                work_timestamp_keys = list(sorted(list(common_motifs_work.keys())))
                work_timestamp_key_to_process = None

                # @added 20250804 - Feature #5644: ionosphere.learn_self_validation
                if common_motifs_validation_work:
                    work_timestamp_keys = list(sorted(list(common_motifs_work.keys())))
                    work_timestamp_key_to_process = work_timestamp_keys[0]
                    work_timestamp_keys = []                    

                for work_timestamp_key in work_timestamp_keys:
                    if (int(float(work_timestamp_key)) + 4200) < current_timestamp:
                        # @added 20240910 - Feature #5318: common_motifs
                        # Remove old keys
                        if int(float(work_timestamp_key)) < (current_timestamp - (3600 * 3)):
                            try:
                                self.redis_conn_decoded.hdel('ionosphere.find_repetitive_patterns.common_motifs.work', work_timestamp_key)
                                logger.info('%s :: hdel old key %s from ionosphere.find_repetitive_patterns.common_motifs.work Redis hash' % (
                                    app_module, work_timestamp_key))
                                continue
                            except Exception as err:
                                logger.error('error :: %s :: failed to hdel old key %s from ionosphere.find_repetitive_patterns.common_motifs.work Redis hash, err: %s' % (
                ...
                # @added 20250922 - Feature #5644: ionosphere.learn_self_validation
                if not work_timestamp_key_to_process:
                    ...
                    try:
                        ...
                        logger.error('error :: could not query Redis hash ionosphere.learn_self_validation.common_motifs.work, err: %s' % err)
                    if common_motifs_validation_work:
                        logger.info('%s :: using ionosphere.find_repetitive_patterns.common_motifs.work' % (
                            app_module))
                        common_motifs_work = copy.deepcopy(common_motifs_validation_work)
                        context_hash_key = 'ionosphere.learn_self_validation.common_motifs.work'
                    work_timestamp_keys = list(sorted(list(common_motifs_work.keys())))
                    if work_timestamp_keys:
                        work_timestamp_key_to_process = work_timestamp_keys[0]
                ...
                try:
                    work_dict = literal_eval(common_motifs_work[work_timestamp_key])
                except Exception as err:
                    # @added 20240910 - Feature #5318: common_motifs
                    if 'malformed node or string' in str(err):
                        work_dict = None
                        # @added 20240922 - Feature #5318: common_motifs
                        if 'nan]' in str(common_motifs_work[work_timestamp_key]):
                            try:
                                data_str = common_motifs_work[work_timestamp_key]
                                work_dict_str = data_str.replace('nan]', 'None]')
                                work_dict = literal_eval(work_dict_str)
                            except Exception as err2:
                                logger.error('error :: %s :: literal_eval failed on common_motifs_work[%s] (with nan replacement str), err: %s' % (
                                    app_module, work_timestamp_key, err2)) 
                    else:
                        logger.error('error :: %s :: literal_eval failed on common_motifs_work[%s], err: %s' % (
                            app_module, work_timestamp_key, err))
                    if not work_dict:
                        try:
                            # self.redis_conn_decoded.hdel('ionosphere.find_repetitive_patterns.common_motifs.work', work_timestamp_key)
                            self.redis_conn_decoded.hdel(context_hash_key, work_timestamp_key)
                            logger.info('%s :: no data from literal_eval on common_motifs_work[%s], deleted from %s' % (
                                app_module, work_timestamp_key, context_hash_key))
                        except Exception as err:
                            logger.error('error :: %s :: failed to hdel literal_eval fail key %s from ionosphere.find_repetitive_patterns.common_motifs.work Redis hash, err: %s' % (
                                app_module, work_timestamp_key, err))
            ...
            if work_dict:
                ...
                if fp_id:
                    ...
                    sent_via_wind = False
                    try:
                        sent_via_wind = self.redis_conn_decoded.exists(wind_fp_id_hash_key)
                    except Exception as err:
                        logger.error('error :: %s :: exists failed on %s, err: %s' % (
                            app_module, wind_fp_id_hash_key, err))            
                    if sent_via_wind:
                        logger.info('%s :: deduplicating work sent via wind for fp_id: %s' % (
                            app_module, str(fp_id)))
                        try:
                            self.redis_conn_decoded.hdel(context_hash_key, work_timestamp_key)
                        except Exception as err:
                            logger.error('error :: %s :: failed to remove failed work key %s from ionosphere.find_repetitive_patterns.common_motifs.work, err: %s' % (
                                app_module, work_timestamp_key, err))
                        work_dict = None

            # @added 20260213 - Feature #5318: common_motifs
            # Prevent common_motifs from learning from consecutive training_data
            # instances.  Limit common_motifs to learning and creating 1 fp for
            # the IONOSPHERE_AUTONOMOUS_LEARN_RATE period.
            # This ensures that if a metirc triggers repeatedly and many
            # consecutive training_data sets exist for the metic, common_motifs
            # will only learn 1 for the IONOSPHERE_AUTONOMOUS_LEARN_RATE
            # period.  This prevents lots of patterns that are extremely similar
            # for being learnt.
            # @modified 20260215 - Feature #5318: common_motifs
            # Only added this comment, no code change. Perhaps we actually want
            # to learn dips and their recovery and setting this default to 12
            # hours stops that from happening.  But maybe that is not so bad as
            # when the metric next triggers after 12 hours (if it does) the dip
            # recovery will be captured in that training data set.  And if dip
            # and recovery are common they will be learnt.  Every learnt dip
            # does not need it's recovery learnt immediately, if common it will
            # be learnt at some point.
            if work_dict and context_hash_key != 'ionosphere.learn_self_validation.common_motifs.work':
                # IONOSPHERE_AUTONOMOUS_LEARN_RATE = 43200
                metric_id = work_dict['metric_id']
                metric = work_dict['metric']
                last_learnt_timestamp = 0
                fps_dict = {}
                try:
                    fps_dict = get_fps_for_metric(skyline_app, metric_id)
                except Exception as err:
                    logger.error('error :: %s :: get_fps_for_metric failed for metric_id: %s, err: %s' % (
                        app_module, str(metric_id), err))
                for fp_id, item in fps_dict.items():
                    if item['generation'] > 1:
                        last_learnt_timestamp = item['anomaly_timestamp']
                anomaly_timestamp = 0
                try:
                    anomaly_timestamp = work_dict['anomaly_timestamp']
                except Exception as err:
                    logger.error('error :: %s :: failed to determine anomaly_timestamp from work_dict, err: %s' % (
                        app_module, err))
                if last_learnt_timestamp and anomaly_timestamp:
                    if (last_learnt_timestamp + IONOSPHERE_AUTONOMOUS_LEARN_RATE) > anomaly_timestamp:
                        logger.info('%s :: IONOSPHERE_AUTONOMOUS_LEARN_RATE period has not expired for %s not attempting to learn from %s training_data set' % (
                            app_module, metric, str(anomaly_timestamp)))
                        try:
                            self.redis_conn_decoded.hdel(context_hash_key, work_timestamp_key)
                        except Exception as err:
                            logger.error('error :: %s :: failed to remove failed work key %s from %s, err: %s' % (
                                app_module, work_timestamp_key, context_hash_key, err))
                        work_dict = None
            ...
            if work_dict:
                ...
                # @added 20250804 - Feature #5644: ionosphere.learn_self_validation
                if 'pw4_timeseries' in log_work_dict.keys():
                    log_work_dict['pw4_timeseries'] = 'pw4_timeseries redacted for log'
                ...
                # Reduce run rate
                try:
                    self.redis_conn_decoded.setex('ionosphere.common_motifs.running', 1200, int(time()))
                except Exception as err:
                    logger.error('error :: %s :: failed to setex ionosphere.common_motifs.running from Redis, err: %s' % (
                        app_module, err))
                ...
                spawned_pids = []
                pid_count = 0
                p = Process(target=self.spin_common_motifs, args=(work_dict,))
                pids.append(p)
                pid_count += 1
                logger.info('%s :: starting spin_common_motifs' % app_module)
                p.start()
                spawned_pids.append(p.pid)

                # Self monitor processes and terminate if any spin_snab_process
                # that has run for longer than 1200 seconds
                p_starts = time()
                while time() - p_starts <= 1200:
                    if any(p.is_alive() for p in pids):
                        ...
                        logger.info('%s :: spin_common_motifs completed in %.2f seconds' % (
                            app_module, time_to_run))
                        break
                else:
                    # We only enter this if we didn't 'break' above.
                    logger.info('%s :: timed out, killing all spin_common_motifs process' % app_module)
                    for p in pids:
                        p.terminate()
                        remove_work = True
                for p in pids:
                    if p.is_alive():
                        logger.info('%s :: stopping spin_common_motifs - %s' % (app_module, str(p.is_alive())))
                        killing_pid = p.pid
                        logger.info('%s :: kill spin_common_motifs with pid: %s' % (app_module, str(killing_pid)))
                        p.terminate()
                        logger.info('%s :: killed spin_common_motifs process with pid: %s' % (app_module, str(killing_pid)))
                        remove_work = True
                if remove_work:
                    logger.info('%s :: removing failed work key %s from %s' % (
                        app_module, work_timestamp_key, context_hash_key))
                    try:
                        #self.redis_conn_decoded.hdel('ionosphere.find_repetitive_patterns.common_motifs.work', work_timestamp_key)
                        self.redis_conn_decoded.hdel(context_hash_key, work_timestamp_key)
                ...
                # Reduce run rate
                try:
                    self.redis_conn_decoded.delete('ionosphere.common_motifs.running')
                except Exception as err:
                    logger.error('error :: %s :: failed to delete ionosphere.common_motifs.running from Redis, err: %s' % (
                        app_module, err))
