...
def histogram_bins(timeseries):
    ...
    try:
        # DEBUG make exception
        # series1 = pandas.Series([x[1] for x in timeseries])
        # last_hour_threshold = time() - (FULL_DURATION - 3600)
        # series2 = pandas.Series([x[1] for x in timeseries if x[0] < last_hour_threshold])
        # expAverage = pandas.Series.ewm(series, ignore_na=False, min_periods=0, adjust=True, com=50).mean()
        # stdDev1 = pandas.Series.ewm(series, ignore_na=False, min_periods=0, adjust=True, com=50).std(bias=False)
        # series = pandas.Series([x[1] if x[1] else 0 for x in timeseries])
        # series4 = series - series[0:len(series) - 1].mean()
        # expAverage = pandas.Series.ewm(series, ignore_na=False, min_periods=0, adjust=True, com=15).mean()
        # debug = pandas
