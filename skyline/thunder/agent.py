...
class ThunderAgent():
    """
    The ThunderAgent class does the following:
...
def run():
    ...
    thunder = ThunderAgent()

    logger.info('starting thunder.run')
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'run':
        ...
        daemon_runner = runner.DaemonRunner(thunder)
        daemon_runner.daemon_context.files_preserve = [handler.stream]
        daemon_runner.do_action()

    logger.info('stopping thunder')
    memory_handler.flush
