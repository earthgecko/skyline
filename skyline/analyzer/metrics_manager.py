...
# @added 20220708 - Info #4620: memray
import memray
...
from skyline_functions import (
    ...
    # @modified 20220630 - Task #2732: Prometheus to Skyline
...
#                   Branch #4300: prometheus
from functions.metrics_manager.manage_prometheus_metadata import manage_prometheus_metadata

# @added 20220627 - Task #2732: Prometheus to Skyline
#                   Branch #4300: prometheus
from functions.metrics_manager.redistimeseries_roomba import redistimeseries_roomba
...
#                   Branch #4300: prometheus
from functions.metrics_manager.prune_expose_prometheus_metrics import prune_expose_prometheus_metrics
from functions.metrics_manager.manage_smtp_alerter_labelled_metrics import manage_smtp_alerter_labelled_metrics

# @added 20220824 - Feature #3870: metrics_manager - check_data_sparsity
# Do not check new metrics
from functions.database.queries.get_new_metrics import get_new_metrics
...
from functions.metrics_manager.purge_vortex_saved_data import purge_vortex_saved_data
...
#                   Branch #4300: prometheus
from functions.metrics_manager.update_labelled_metrics_data_sparsity import update_labelled_metrics_data_sparsity

# @added 20230505 - Feature #4848: mirage - analyse.irregular.unstable.timeseries.at.30days
from functions.metrics_manager.skip_irregular_unstable_metric_ids import build_skip_irregular_unstable_metric_ids

# @added 20230510 - Feature #4902: Prevent training on metrics newer than 7 days
from functions.metrics_manager.manage_untrainable_new_metrics import manage_untrainable_new_metrics

# @added 20230519 - metric_type.longterm_expire
from functions.metrics_manager.manage_labelled_metrics_longterm_metric_type import manage_labelled_metrics_longterm_metric_type
...
# Prune the mirage.vortex and flux.vortex hash keys
try:
...
class Metrics_Manager(Thread):
    ...
    def sync_cluster_files(self, i):
        ...
        if not local_skyline_instance:
            logger.info('metrics_manager :: sync_cluster_files could not determine local_skyline_instance from REMOTE_SKYLINE_INSTANCES, created')
            skyline_url = 'http://%s:%s' % (settings.WEBAPP_IP, str(settings.WEBAPP_PORT))
            if settings.WEBAPP_AUTH_ENABLED:
                local_skyline_instance = [
                    skyline_url, settings.WEBAPP_AUTH_USER,
        ...
        for remote_skyline_instance in REMOTE_SKYLINE_INSTANCES:
            ...
            if remote_training_data:
                ...
                for remote_training_data_item in remote_training_data:
                    ...
                    #                   Branch #4300: prometheus
                    # Handle labelled_metrics, there are entries for training data for
                    # both the labelled_metrics.<METRIC_ID> and the labelled metric base_name
                    # in terms of syncing, only the labelled_metrics.<METRIC_ID> entry needs
                    # to be handled
                    if '_tenant_id="' in remote_training_data_item[0]:
        ...
        if len(local_features_profile_dirs) == len(expected_features_profile_dirs):
            ...
            for expected_features_profile_dir in expected_features_profile_dirs:
                ...
                try:
                    if '_tenant_id="' in expected_features_profile_dirs[expected_features_profile_dir]:
                        logger.info('metrics_manager :: skipping %s' % str(expected_features_profile_dirs[expected_features_profile_dir]))
                        continue
                except Exception as err:
                    logger.error('error :: metrics_manager :: failed to determine if _tenant_id in expected_features_profile_dirs[expected_features_profile_dir] - %s' % err)
                ...
                for remote_skyline_instance in USE_REMOTE_SKYLINE_INSTANCES:
                    ...
                    if features_profile_files:
                        ...
                        if (files_fetched + files_present) == files_to_fetch:
                            logger.info('metrics_manager :: sync_cluster_files - got all %s features_profile files that needed to be fetchd (%s were already present) for %s from %s' % (
                            ...
                            # in terms of syncing, only the labelled_metrics.<METRIC_ID> entry needs
                            # to be handled
                            if '_tenant_id="' in str(expected_dir):
                                continue
        ...
        # keys if they have been removed recently
        quota_metrics_removed = False
        ...
        for parent_namespace in list(metrics_manager_flux_namespace_quotas.keys()):
            ...
            # Handle labelled_metrics, do not sync the flux.quota.namespace_metrics
            # keys if they have been removed recently
            namespaces_quota_removed_key = 'flux.removed.quota.namespace_metrics.%s' % str(parent_namespace)
            try:
                quota_metrics_removed = self.redis_conn.exists(namespaces_quota_removed_key)
            except Exception as err:
                logger.error('error :: metrics_manager :: sync_cluster_files :: exists failed on %s Redis key - %s' % (
                    namespaces_quota_removed_key, err))
            if quota_metrics_removed:
                logger.info('metrics_manager :: sync_cluster_files :: %s exists so skipping syncing namesapce quota this run' % namespaces_quota_removed_key)
                continue
    ...
    def metric_management_process(self, i):
        ...
        # @added 20220708 - Info #4620: memray
        memray_file = '%s/analyzer.metrics_manager_process.bin' % settings.SKYLINE_TMP_DIR
        memray_file_last = '%s.last' % memray_file
        if os.path.isfile(memray_file_last):
            os.remove(memray_file_last)
            logger.info('metrics_manager :: removed %s' % str(memray_file_last))
        ...
        try:
            with memray_tracker:
                ...
                #                   Feature #4324: flux - reload external_settings
                #                   Feature #4376: webapp - update_external_settings
                # @modified 20220512 - Feature #4324: flux - reload external_settings
                #                      Feature #4376: webapp - update_external_settings
                # Moved block to very beginning so that reload_flux is done as soon as
                # possible
                ...
                if last_run_timestamp:
                    # @modified 20220512 - Feature #4324: flux - reload external_settings
                    #                      Feature #4376: webapp - update_external_settings
                    # If there is a reload_flux called then do not return but run
                    if not do_reload_flux:
                        logger.info('metrics_manager :: analyzer.metrics_manager.last_run_timestamp Redis key has not expired, not running')
                        return
                    else:
                        logger.info('metrics_manager :: analyzer.metrics_manager.last_run_timestamp Redis key has not expired, but reload_flux called so running')

                # @added 20220919 - Feature #4676: analyzer - illuminance.all key
                # Create a Redis hash with the algorithm ids
                algorithms = {}
                try:
                ...
                # labelled_metrics.unique_metrics
                labelled_metrics_active_ids = []
                labelled_metrics_unique_metrics = []
                try:
                    labelled_metrics_unique_metrics = list(self.redis_conn_decoded.smembers('labelled_metrics.unique_labelled_metrics'))
                except Exception as err:
                    logger.error(traceback.format_exc())
                    logger.error('error :: metrics_manager :: smembers failed on labelled_metrics.unique_labelled_metrics - %s' % err)
                ...
                #                   Task #2732: Prometheus to Skyline
                #                   Branch #4300: prometheus
                # Added the ability for boundary to handle labelled_metrics
                # via metrics_manager.boundary_metrics
                active_labelled_metrics_with_id = {}
                try:
                    active_labelled_metrics_with_id = self.redis_conn_decoded.hgetall('aet.metrics_manager.active_labelled_metrics_with_id')
                except Exception as err:
                    logger.error('error :: metrics_manager :: failed to hgetall Redis hash key aet.metrics_manager.active_labelled_metrics_with_id for boundary_metrics - %s' % (
                        err))

                # Check if this process is unnecessary
                if len(unique_metrics) == 0 and len(labelled_metrics_unique_metrics):
                ...
                logger.info('metrics_manager :: created all_active_base_names list of %s metrics' % str(len(all_active_base_names)))
                ...
                if not refresh_redis_alert_sets:
                    ...
                    if known_alerter_metrics_set:
                        ...
                        try:
                            ...
                            logger.error(traceback.format_exc())
                            logger.error('error :: metrics_manager :: failed to determine whether the unique_base_names_set and known_alerter_metrics_set are different')
                ...
                if refresh_redis_alert_sets:
                    ...
                    for base_name in unique_base_names:
                        ...
                        #                      Feature #4446: Optimise horizon worker in_skip_list
                        # Commented out the if base_name not in all_smtp_alerter_metrics
                        # because it is a unique list so each item will only be iterated
                        # once. Unindented to for block below
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after set management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                if EXTERNAL_SETTINGS:
                    ...
                    # @added 20220512- Feature #3824: get_cluster_data
                    #                  Release #4562 - v3.0.4
                    #                  Feature #4324: flux - reload external_settings
                    #                  Feature #4376: webapp - update_external_settings
                    # Do reload_flux as soon as possible
                    if do_reload_flux:
                        ...
                        # @added 20220825- Feature #4324: flux - reload external_settings
                        # Save the external_settings that flux will be restarted
                        # with for debugging purposes
                        backup_redis_key = 'skyline.external_settings.flux_reload.%s' % str(int(time()))
                        logger.info('metrics_manager :: saving external_settings to %s with %s' % (
                            backup_redis_key, str(external_settings)))
                        try:
                            self.redis_conn_decoded.setex(backup_redis_key, (86400 * 14), str(external_settings))
                        except Exception as err:
                            logger.error('error :: metrics_manager :: failed to set %s - %s' % (
                                backup_redis_key, err))
                ...
                if do_not_skip_metrics_dict and settings.DO_NOT_SKIP_LIST:
                    logger.info('metrics_manager :: determining new metrics to add to horizon.do_not_skip_metrics')
                    start_horizon_do_not_skip_metrics = time()
                    all_horizon_metrics = list(set(unique_base_names + list(skip_metrics_dict.keys())))
                    update_do_not_skip_metrics = {}

                    # @modified 20230316 - Task #4872: Optimise luminosity for labelled_metrics
                    #                      Feature #4446: Optimise horizon worker in_skip_list
                    # More efficient to get the set difference
                    # for base_name in all_horizon_metrics:
                    #     if base_name in horizon_do_not_skip_metrics:
                    #         continue
                    all_horizon_metrics_set = set(all_horizon_metrics)
                    horizon_do_not_skip_metrics_set = set(horizon_do_not_skip_metrics)
                    all_horizon_metrics_to_check = list(all_horizon_metrics_set.difference(horizon_do_not_skip_metrics_set))
                    del all_horizon_metrics_set
                    del horizon_do_not_skip_metrics_set
                    for base_name in all_horizon_metrics_to_check:
                        ...
                        del metric_matched_by
                    del all_horizon_metrics_to_check

                    new_do_not_skip_metrics = list(update_do_not_skip_metrics.keys())
                    logger.info('metrics_manager :: found %s new metrics to add to horizon.do_not_skip_metrics Redis hash, took %s seconds' % (
                        str(len(new_do_not_skip_metrics)), str(time() - start_horizon_do_not_skip_metrics)))
                ...
                #                   Branch #4300: prometheus
                logger.info('metrics_manager :: getting last aet.metrics_manager.metric_names_with_ids from Redis')
                ...
                try:
                    ...
                    logger.info('metrics_manager :: got %s last aet.metrics_manager.metric_names_with_ids from Redis' % str(len(aet_metric_names_with_ids)))
                except Exception as err:
                    logger.error('metrics_manager :: failed to hgetall aet.metrics_manager.metric_names_with_ids - %s' % err)
                known_prometheus_metrics = {}
                if aet_metric_names_with_ids:
                    for metric_id in list(aet_metric_names_with_ids.keys()):
                        known_metric = aet_metric_names_with_ids[metric_id]
                        if '{' in known_metric and '}' in known_metric and '="' in known_metric:
                            known_prometheus_metrics[known_metric] = metric_id
                current_prometheus_skip_dict = {}
                prometheus_skip_dict = {}
                prometheus_skip_dict_changed = False
                if known_prometheus_metrics:
                    logger.info('metrics_manager :: managing metrics_manager.prometheus_skip_dict for %s prometheus metrics' % str(len(known_prometheus_metrics)))
                    current_prometheus_skip_dict_data = {}
                    try:
                        current_prometheus_skip_dict_data = self.redis_conn_decoded.hgetall('metrics_manager.prometheus_skip_dict')
                    except Exception as err:
                        logger.error('error :: metrics_manager :: failed to hset metrics_manager.prometheus_skip_dict - %s' % (
                            err))
                    # Convert str
                    for metric in list(current_prometheus_skip_dict_data.keys()):
                        current_prometheus_skip_dict[metric] = int(str(current_prometheus_skip_dict_data[metric]))
                    del current_prometheus_skip_dict_data

                # Thoughts instead of a 1 or 0 add the type and if to be skipped set as
                # skip or None.  That can then be used to determine the aggregation
                # method.  Introduces a problem of the source and fetching all the
                # targets/metadata ... Solved by metadata_config

                do_not_skip_prometheus_metrics_count = 0
                for metric in list(known_prometheus_metrics.keys()):
                    skip = 0
                    pattern_match = False
                    try:
                        pattern_match, metric_matched_by = matched_or_regexed_in_list(skyline_app, metric, ALL_SKIP_LIST)
                        del metric_matched_by
                    except:
                        pattern_match = False
                    if pattern_match:
                        ...
                        do_not_skip_prometheus_metrics_count += 1
                    prometheus_skip_dict[metric] = skip
                if prometheus_skip_dict:
                    for metric in list(prometheus_skip_dict.keys()):
                        if prometheus_skip_dict_changed:
                            break
                        try:
                            if prometheus_skip_dict[metric] != current_prometheus_skip_dict[metric]:
                                prometheus_skip_dict_changed = True
                        except:
                            prometheus_skip_dict_changed = True
                    for metric in list(current_prometheus_skip_dict.keys()):
                        if prometheus_skip_dict_changed:
                            break
                        try:
                            if prometheus_skip_dict[metric] != current_prometheus_skip_dict[metric]:
                ...
                if prometheus_skip_dict_changed:
                    try:
                        self.redis_conn_decoded.hset('metrics_manager.prometheus_skip_dict', mapping=prometheus_skip_dict)
                    except Exception as err:
                        logger.error('error :: metrics_manager :: failed to hset metrics_manager.prometheus_skip_dict - %s' % (
                            err))
                    logger.info('metrics_manager :: updated metrics_manager.prometheus_skip_dict with %s skip and %s do_not_skip prometheus metrics' % (
                        str(len(known_prometheus_metrics) - do_not_skip_prometheus_metrics_count),
                        str(do_not_skip_prometheus_metrics_count)))
                    try:
                        self.redis_conn_decoded.set('metrics_manager.updated.prometheus_skip_dict', int(time()))
                    except Exception as err:
                        logger.error('error :: metrics_manager :: failed to set metrics_manager.updated.prometheus_skip_dict - %s' % (
                            err))
                # Remove inactive metrics from the metrics_manager.prometheus_skip_dict
                # Redis hash
                remove_inactive_from_prometheus_skip_dict = []
                for metric in list(current_prometheus_skip_dict.keys()):
                    metric_active = 0
                    try:
                        metric_active = str(known_prometheus_metrics[metric])
                    except:
                        remove_inactive_from_prometheus_skip_dict.append(metric)
                    del metric_active
                logger.info('metrics_manager :: found %s inactive metrics to remove from the metrics_manager.prometheus_skip_dict Redis hash' % (
                    str(len(remove_inactive_from_prometheus_skip_dict))))
                if remove_inactive_from_prometheus_skip_dict:
                    try:
                        removed_count = self.redis_conn_decoded.hdel('metrics_manager.prometheus_skip_dict', *remove_inactive_from_prometheus_skip_dict)
                        logger.info('metrics_manager :: removed %s inactive metrics from the metrics_manager.prometheus_skip_dict Redis hash' % (
                            str(removed_count)))
                    except Exception as err:
                        logger.error('error :: metrics_manager :: failed to remove inactive metrics from metrics_manager.prometheus_skip_dict Redis hash - %s' % (
                            err))
                del current_prometheus_skip_dict
                del known_prometheus_metrics
                del prometheus_skip_dict
                ...
                try:
                    prometheus_metadata_dict, prometheus_metrics_type_dict = manage_prometheus_metadata(self)
                except Exception as err:
                    logger.error('error :: metrics_manager :: manage_prometheus_metadata failed - %s' % (
                        err))
                try:
                    del prometheus_metadata_dict
                    del prometheus_metrics_type_dict
                except:
                    pass

                # @added 20220627 - Task #2732: Prometheus to Skyline
                #                   Branch #4300: prometheus
                # Run redistimeseries_roomba
#                logger.info('metrics_manager :: running redistimeseries_roomba')
#                roombaed_redistimeseries_metrics = []
#                try:
#                    roombaed_redistimeseries_metrics = redistimeseries_roomba(self, active_labelled_ids_with_metrics, ids_with_labelled_metric_names)
#                except Exception as err:
#                    logger.error(traceback.format_exc())
#                    logger.error('error :: metrics_manager :: redistimeseries_roomba failed - %s' % (
#                        err))
#                logger.info('metrics_manager :: redistimeseries_roomba removed %s labelled_metrics' % str(len(roombaed_redistimeseries_metrics)))
#                try:
#                    del roombaed_redistimeseries_metrics
#                except:
#                    pass

                logger.info('metrics_manager :: pruning old entries from analyzer_labelled_metrics.last_timeseries_timestamp')
                analyzer_labelled_metrics_last_timeseries_timestamps = {}
                try:
                    analyzer_labelled_metrics_last_timeseries_timestamps = self.redis_conn_decoded.hgetall('analyzer_labelled_metrics.last_timeseries_timestamp')
                except Exception as err:
                    logger.error(traceback.format_exc())
                    logger.error('error :: metrics_manager :: hget analyzer_labelled_metrics.last_timeseries_timestamp failed - %s' % err)
                oldest_timestamp = int(spin_start) - settings.FULL_DURATION
                removed_ts_keys = 0
                remove_keys = []
                for metric_id_str in list(analyzer_labelled_metrics_last_timeseries_timestamps.keys()):
                    try:
                        ts = int(float(analyzer_labelled_metrics_last_timeseries_timestamps[metric_id_str]))
                        if ts < oldest_timestamp:
                            remove_keys.append(metric_id_str)
                    except Exception as err:
                        logger.error(traceback.format_exc())
                        logger.error('error :: metrics_manager :: failed to evaluate %s timestamp from analyzer_labelled_metrics.last_timeseries_timestamp - %s' % (
                            metric_id_str, err))
                del analyzer_labelled_metrics_last_timeseries_timestamps
                if remove_keys:
                    try:
                        removed_keys = self.redis_conn_decoded.hdel('analyzer_labelled_metrics.last_timeseries_timestamp', *remove_keys)
                        if removed_keys:
                            removed_ts_keys += removed_keys
                    except Exception as err:
                        logger.error(traceback.format_exc())
                        logger.error('error :: metrics_manager :: hdel from analyzer_labelled_metrics.last_timeseries_timestamp failed - %s' % (
                            err))
                logger.info('metrics_manager :: pruned %s old entries from analyzer_labelled_metrics.last_timeseries_timestamp' % str(removed_ts_keys))
                del removed_ts_keys
                del remove_keys

                logger.info('metrics_manager :: metric_management_process :: memory usage after labelled_metrics management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                #                      Feature #4464: flux - quota - cluster_sync
                # Added this comment only - this is managed from a removal of
                # metrics point of view in the skyline/functions/flux/remove_namespace_quota_metrics.py
                # along with the normal webapp admin API method of
                # /remove_namespace_quota_metrics
                for parent_namespace in list(namespace_quotas_dict.keys()):
                    ...
                    # @added 20230322 - Feature #4468: flux - remove_namespace_quota_metrics
                    #                   Feature #4464: flux - quota - cluster_sync
                    # Do not manage if things have been removed
                    quota_metrics_removed = False
                    if namespace_metrics:
                        ...
                        try:
                            ...
                            logger.error('error :: metrics_manager :: exists failed on Redis key %s - %s' % (
                                namespaces_quota_removed_key, err))
                    if quota_metrics_removed:
                        logger.info('metrics_manager :: quota_metrics_removed exists so skipping managing metrics_manager.flux.quota.namespace_metrics')
                        continue
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after namespace quota management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                if FLUX_AGGREGATE_NAMESPACES or FLUX_EXTERNAL_AGGREGATE_NAMESPACES or external_settings:
                    if FLUX_AGGREGATE_NAMESPACES:
                        ...
                        # @added 20220720 - Feature #3708: FLUX_ZERO_FILL_NAMESPACES
                        # Add any aggregation metrics to the relevant lists
                        for aggregate_namespace in aggregate_namespaces:
                            zero_fill_set = False
                            try:
                                zero_fill_set = FLUX_AGGREGATE_NAMESPACES[aggregate_namespace]['zero_fill']
                            except:
                                zero_fill_set = False
                            if zero_fill_set:
                                if aggregate_namespace not in FLUX_ZERO_FILL_NAMESPACES:
                                    FLUX_ZERO_FILL_NAMESPACES.append(aggregate_namespace)
                                    logger.info('metrics_manager :: FLUX_ZERO_FILL_NAMESPACES - added aggregate_namespace: %s' % str(aggregate_namespace))
                            lkv_set = False
                            try:
                                lkv_set = FLUX_AGGREGATE_NAMESPACES[aggregate_namespace]['last_known_value']
                            except:
                                lkv_set = False
                            if lkv_set:
                                if aggregate_namespace not in FLUX_LAST_KNOWN_VALUE_NAMESPACES:
                                    FLUX_LAST_KNOWN_VALUE_NAMESPACES.append(aggregate_namespace)
                    ...
                    if external_settings_aggregations:
                        for namespace_key in list(external_settings_aggregations.keys()):
                            ...
                            FLUX_AGGREGATE_NAMESPACES[namespace_key] = copy.deepcopy(external_settings_aggregations[namespace_key])
                        ...
                        for i_base_name in unique_base_names:
                            ...
                            if pattern_match:
                                ...
                                if add_to_zero_fill:
                                    ...
                                    # logger.debug('debug :: metrics_manager :: adding i_base_name: %s to flux_aggregate_zerofill_metrics' % str(i_base_name))
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after aggregate management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                        ...
                        if namespace in flux_namespaces:
                            ...
                            if redis_hash == 'flux.skipped_metrics':
                                ...
                                for metric in metrics_manager_skip_set:
                                    ...
                                    if metric.startswith(full_parent_namespace):
                ...
                #                   Feature #3870: metrics_manager - check_data_sparsity
                # Add custom stale periods declared in external settings to
                # CUSTOM_STALE_PERIOD
                if not external_settings:
                    try:
                        ...
                        logger.error('error :: metrics_manager :: get_external_settings failed - %s' % (
                            err))
                for key in list(external_settings.keys()):
                    try:
                        if external_settings[key]['alert_on_stale_metrics']['enabled']:
                            if external_settings[key]['alert_on_stale_metrics']['stale_period'] != settings.STALE_PERIOD:
                                namespace = external_settings[key]['namespace']
                                CUSTOM_STALE_PERIOD[namespace] = external_settings[key]['alert_on_stale_metrics']['stale_period']
                    except Exception as err:
                        logger.error(traceback.format_exc())
                ...
                if CUSTOM_STALE_PERIOD:
                    ...
                    for i_metric in all_active_base_names:
                ...
                if custom_stale_periods:
                    ...
                    logger.info('metrics_manager :: managing %s metrics in %s hash key' % (
                        str(len(custom_stale_periods)), custom_stale_metrics_hash_key))
                    for i_metric, i_stale_period in custom_stale_periods:
                        ...
                        if update_hash_key:
                            try:
                                ...
                                logger.error(traceback.format_exc())
                                logger.error('error :: metrics_manager :: failed to add entry in %s for - %s - %s' % (
                                    custom_stale_metrics_hash_key, str(i_metric), err))
                    ...
                    if metrics_to_remove_from_custom_stale_period_hash:
                        try:
                            ...
                            logger.error(traceback.format_exc())
                            logger.error('error :: metrics_manager :: failed to remove %s metrics from the Redis hash key %s - %s' % (
                ...
                del known_custom_stale_metrics_dict
                del known_custom_stale_metrics
                logger.info('metrics_manager :: custom_stale_period management took %s seconds' % (
                    str(time() - start_custom_stale_period)))
                ...
                try:
                    # @modified 20230523 - Task #2732: Prometheus to Skyline
                    #                      Branch #4300: prometheus
                    # Handle labelled_metrics
                    # do_not_alert_on_stale_metrics_list = do_not_alert_on_stale_metrics(self, external_settings, unique_base_names)
                    do_not_alert_on_stale_metrics_list = do_not_alert_on_stale_metrics(self, external_settings, all_active_base_names)
                except Exception as err:
                    logger.error(traceback.format_exc())
                ...
                if settings.BOUNDARY_METRICS:
                    ...
                    # Added the ability for boundary to handle labelled_metrics
                    # via metrics_manager.boundary_metrics
                    logger.info('metrics_manager :: creating metrics_manager.boundary_metrics')
                    if not active_labelled_metrics_with_id:
                        active_labelled_metrics_with_id = {}
                        try:
                            active_labelled_metrics_with_id = self.redis_conn_decoded.hgetall('aet.metrics_manager.active_labelled_metrics_with_id')
                        except Exception as err:
                    ...
                    #                   Task #2732: Prometheus to Skyline
                    #                   Branch #4300: prometheus
                    # The original method of all metrics being matched_or_regexed_in_list
                    # against all settings.BOUNDARY_METRICS was quite expensive,
                    # taking 15 seconds for 8740 metrics and 62 boundary metric
                    # alert rules.  Still iterating all the metric names and
                    ...
                    #                      Task #2732: Prometheus to Skyline
                    #                      Branch #4300: prometheus
                    # Include labelled_metric base_names as well.  This is quite
                    # expensive, taking 15 seconds for 8740 metrics
                    # for base_name in unique_base_names:
                    for base_name in all_active_base_names:
                        try:
                            ...
                            # the boundary metric alert rules but altogether first and
                            # only checking if any match reduces this to 3 seconds
                            pattern_match, metric_matched_by = matched_or_regexed_in_list(skyline_app, base_name, boundary_metric_patterns)
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after lkv, zerofill, non_derivative management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after namespace_analysed_events management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after low priority management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                if CHECK_DATA_SPARSITY:
                    ...
                    new_metrics = {}
                    try:
                        new_metrics = get_new_metrics(skyline_app)
                    except Exception as err:
                        logger.error('error :: metrics_manager :: get_new_metrics failed - %s' % err)
                    removed_new_metrics_from_check_metrics = 0
                    if new_metrics:
                        for base_name in list(new_metrics.keys()):
                            if '_tenant_id="' in base_name:
                                continue
                            metric_name = '%s%s' % (settings.FULL_NAMESPACE, base_name)
                            try:
                                # @modified 20230126 - Feature #3870: metrics_manager - check_data_sparsity
                                #                      Feature #3820: HORIZON_SHARDS
                                # Added conditional to only remove if in the
                                # list of unique_metrics, on cluster may not be
                                # if the metric is not assigned to this shard
                                # host.
                                if metric_name in check_metrics:
                        ...
                        try:
                            ...
                            if update_metrics_resolutions_key:
                                if last_known_metric_resolution is not None:
                                    ...
                                    # @added 20230315 - Feature #4148: analyzer.metrics_manager.resolutions
                                    # Keep the sane value
                                    update_value = True
                                    if last_known_metric_resolution == 60 and metric_resolution > 60 and metric_resolution < 300:
                    ...
                    if metrics_fully_populated:
                        ...
                        try:
                            send_graphite_metric(self, skyline_app, send_metric_name, str(metrics_fully_populated_count))
                    ...
                    if metrics_sparsity_decreasing:
                        ...
                        try:
                            send_graphite_metric(self, skyline_app, send_metric_name, str(metrics_sparsity_decreasing_count))
                    ...
                    if metrics_sparsity_increasing:
                        ...
                        try:
                            send_graphite_metric(self, skyline_app, send_metric_name, str(metrics_sparsity_increasing_count))
                    ...
                    if avg_sparsity:
                        ...
                        try:
                            send_graphite_metric(self, skyline_app, send_metric_name, str(avg_sparsity))
                    ...
                    try:
                        ...
                        if update_metrics_resolutions_key:
                            if last_known_metric_resolution is not None:
                                ...
                                # @added 20230511 - Feature #4148: analyzer.metrics_manager.resolutions
                                # Keep the sane value
                                if last_known_metric_resolution == 60 and metric_resolution > 60 and metric_resolution < 300:
                                    metric_resolution = 60
                                if last_known_metric_resolution == 60 and metric_resolution < 60:
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after sparisty and resolution management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after last_analyzed_timestamp management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                redis_hashes = [
                    'mirage.panorama.not_anomalous_metrics',
                    'mirage_labelled_metrics.panorama.not_anomalous_metrics'
                ]
                for redis_hash in redis_hashes:
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after ionosphere, panorama and cloudbursts management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))

                # @added 20230510 - Feature #4902: Prevent training on metrics newer than 7 days
                logger.info('metrics_manager :: managing untrainable new metrics')
                new_from = int(time()) - (RUN_EVERY + 60)
                last_full_new_metrics = False
                try:
                    last_full_new_metrics = self.redis_conn_decoded.get('metrics_manager.last_full_new_metrics_timestamp')
                except Exception as err:
                    logger.error('error :: metrics_manager :: get failed on Redis key metrics_manager.last_full_new_metrics_timestamp - %s' % (
                        err))
                if not last_full_new_metrics:
                    new_now = int(time())
                    new_from = new_now - (86400 * 7)
                    logger.info('metrics_manager :: hourly retrieval of new metrics in past 7 days')
                    try:
                        self.redis_conn_decoded.setex('metrics_manager.last_full_new_metrics_timestamp', 3600, str(new_now))
                    except Exception as err:
                        logger.error('error :: metrics_manager :: setex failed on Redis key metrics_manager.last_full_new_metrics_timestamp - %s' % (
                            err))
                try:
                    new_metrics = get_new_metrics(skyline_app, from_timestamp=new_from)
                except Exception as err:
                    logger.error('error :: metrics_manager :: get_new_metrics failed - %s' % (
                        err))
                managed_new_metrics_dict = {'added': None, 'removed': None}
                try:
                    managed_new_metrics_dict = manage_untrainable_new_metrics(self, new_metrics)
                except Exception as err:
                    logger.error('error :: metrics_manager :: manage_untrainable_new_metrics failed - %s' % (
                        err))
                ...
                # @added 20230505 - Feature #4848: mirage - analyse.irregular.unstable.timeseries.at.30days
                skip_irregular_unstable_metric_ids_count = []
                start_skip_irregular_unstable_metric_ids = time()
                try:
                    skip_irregular_unstable_metric_ids_count = build_skip_irregular_unstable_metric_ids(self, metric_names_with_ids, external_settings)
                except Exception as err:
                    logger.error(traceback.format_exc())
                    logger.error('error :: metrics_manager :: build_skip_irregular_unstable_metric_ids failed - %s' % err)
                logger.info('metrics_manager :: determined %s skip_irregular_unstable_metric_ids in %s seconds' % (
                    str(skip_irregular_unstable_metric_ids_count), str(time() - start_skip_irregular_unstable_metric_ids)))

                # @added 20230203 - Feature #4840: flux - prometheus - x-test-only header
                # Readded all_db_metric_names_with_ids some that labelled_metrics
                # base_names can be looked up if they become active again in
                # labelled_metrics.unique_metrics
                all_db_metric_names = []
                ...
                for metric in list(all_db_metric_names_with_ids.keys()):
                    ...
                    # Create a unique list of namespaces to create a hash to be used in
                    # luminosity
                    start_unique_parent_namespaces_timing = time()
                    if '_tenant_id="' in metric:
                        metric_elements = metric.split('{', 1)
                        metric_labels = metric_elements[1]
                        metric_labels = metric_labels.split(',')
                        tenant_id_label = metric_labels[0]
                        tenant_id_label_list = tenant_id_label.split('"')
                        parent_namespace = tenant_id_label_list[1]
                    else:
                        parent_namespace = metric.split('.')[0]
                    active_namespace = False
                    try:
                        active_namespace = metric_names_with_ids[metric]
                    except:
                        active_namespace = False
                    if active_namespace:
                        try:
                ...
                # Create a unique list of namespaces to create a hash to be used in
                # luminosity
                known_parent_namespaces = []
                try:
                    known_parent_namespaces = self.redis_conn_decoded.hkeys('metrics_manager.luminosity.namespaces')
                except Exception as err:
                    logger.error('error :: metrics_manager :: hkeys failed on metrics_manager.luminosity.namespaces Redis hash - %s' % str(err))
                add_luminosity_namespace_keys = {}
                for unique_parent_namespace in list(unique_parent_namespaces.keys()):
                    if unique_parent_namespace not in known_parent_namespaces:
                        logger.info('metrics_manager :: adding namespace: %s to add_luminosity_namespace_keys' % str(unique_parent_namespace))
                        add_luminosity_namespace_keys[unique_parent_namespace] = 0
                        known_parent_namespaces.append(unique_parent_namespace)
                if add_luminosity_namespace_keys:
                    logger.info('metrics_manager :: adding %s namespaces to metrics_manager.luminosity.namespaces Redis hash' % str(len(add_luminosity_namespace_keys)))
                    logger.info('metrics_manager :: adding add_luminosity_namespace_keys: %s' % str(add_luminosity_namespace_keys))
                    try:
                        known_parent_namespaces_added = self.redis_conn_decoded.hset('metrics_manager.luminosity.namespaces', mapping=add_luminosity_namespace_keys)
                        logger.info('metrics_manager :: %s namespace added to metrics_manager.luminosity.namespaces' % str(known_parent_namespaces_added))
                    except Exception as err:
                        logger.error('error :: metrics_manager :: hset failed on metrics_manager.luminosity.namespaces Redis hash - %s' % str(err))
                try:
                    self.redis_conn_decoded.hset('metrics_manager.luminosity.namespaces', mapping=unique_parent_namespaces)
                    logger.info('metrics_manager :: updated %s namespace counts in metrics_manager.luminosity.namespaces' % str(len(unique_parent_namespaces)))
                except Exception as err:
                    logger.error('error :: metrics_manager :: hset failed on metrics_manager.luminosity.namespaces Redis hash - %s' % str(err))
                remove_namespace_keys = []
                try:
                    known_parent_namespaces = self.redis_conn_decoded.hkeys('metrics_manager.luminosity.namespaces')
                except Exception as err:
                    logger.error('error :: metrics_manager :: hkeys failed on metrics_manager.luminosity.namespaces Redis hash - %s' % str(err))
                for known_parent_namespace in list(known_parent_namespaces):
                    # Remove inactive namespaces
                    if known_parent_namespace not in list(unique_parent_namespaces.keys()):
                        logger.info('metrics_manager :: removing %s namespace from metrics_manager.luminosity.namespaces' % str(known_parent_namespaces))
                        try:
                            self.redis_conn_decoded.hdel('metrics_manager.luminosity.namespaces', known_parent_namespace)
                        except Exception as err:
                            logger.error('error :: metrics_manager :: hdel failed with %s on metrics_manager.luminosity.namespaces Redis hash - %s' % (
                                str(known_parent_namespace), str(err)))
                unique_parent_namespaces_timing = sum(unique_parent_namespaces_timings) / len(unique_parent_namespaces_timings)
                logger.info('metrics_manager :: metrics_manager.luminosity.namespaces managed, unique_parent_namespaces_timings: %s seconds, %s namespaces determined from %s metrics' % (
                    str(unique_parent_namespaces_timing), str(len(unique_parent_namespaces)),
                    str(len(all_db_metric_names_with_ids))))
                ...
                if metric_names_with_ids:
                    ...
                    # aet_metric_names_with_ids = {}
                    if not aet_metric_names_with_ids:
                        try:
                            aet_metric_names_with_ids = self.redis_conn_decoded.hgetall('aet.metrics_manager.metric_names_with_ids')
                ...
                # Redis for metrics_manager to reset as active
                logger.info('metrics_manager :: checking if horizon.prometheus has added any metrics to horizon.active_metrics_not_in_metric_names_with_ids')
                metric_ids_to_reactive = []
                try:
                    metric_ids_to_reactive = list(self.redis_conn_decoded.smembers('horizon.active_metrics_not_in_metric_names_with_ids'))
                    logger.info('metrics_manager :: horizon.prometheus added %s metric_idsto reactive' % str(len(metric_ids_to_reactive)))
                except Exception as err:
                ...
                if metric_ids_to_reactive:
                    try:
                        ...
                        logger.info('metrics_manager :: deleted horizon.active_metrics_not_in_metric_names_with_ids')
                    except Exception as err:
                        logger.error('error :: metrics_manager :: delete on horizon.active_metrics_not_in_metric_names_with_ids failed - %s' % (
                            str(err)))
                    try:
                        set_metrics_as_active_count = set_metric_ids_as_active(skyline_app, metric_ids_to_reactive)
                    except Exception as err:
                ...
                mute_alerts_on_dict = {}
                try:
                    mute_alerts_on_dict, removed = manage_mute_alerts_on(self)
                except Exception as err:
                    logger.error('error :: metrics_manager :: manage_mute_alerts_on failed - %s' % (
                        str(err)))

                ids_with_metric_names = {}
                ids_with_labelled_metric_names = {}
                active_labelled_ids_with_metrics = {}
                ...
                if metric_names_with_ids:
                    ...
                    try:
                        set_len = self.redis_conn.hlen('aet.metrics_manager.metric_names_with_ids')
                        logger.info('metrics_manager :: created the aet.metrics_manager.metric_names_with_ids Redis set with %s metrics' % str(set_len))
                    except Exception as err:
                        logger.error('metrics_manager :: hlen failed on the aet.metrics_manager.metric_names_with_ids Redis set - %s' % err)
                    try:
                    ...
                    # @added 20220620 - Task #2732: Prometheus to Skyline
                    #                   Branch #4300: prometheus
                    logger.info('metrics_manager :: creating active_labelled_metrics_with_id')
                    active_labelled_metrics_with_id = {}
                    analyzer_labelled_metrics_last_timeseries_timestamps = {}
                    try:
                        analyzer_labelled_metrics_last_timeseries_timestamps = self.redis_conn_decoded.hgetall('analyzer_labelled_metrics.last_timeseries_timestamp')
                        ...
                        logger.error(traceback.format_exc())
                        logger.error('error :: metrics_manager :: active_labelled_metrics_with_id - hget analyzer_labelled_metrics.last_timeseries_timestamp failed - %s' % err)
                    ...
                    for c_metric_name in list(metric_names_with_ids.keys()):
                        ...
                        if '{' in c_metric_name and '=' in c_metric_name and '_tenant_id="' in c_metric_name:
                            ids_with_labelled_metric_names[c_metric_id] = c_metric_name

                            # @added 20220620 - Task #2732: Prometheus to Skyline
                            #                   Branch #4300: prometheus
                            # @modified 20230202 - Feature #4792: functions.metrics_manager.manage_inactive_metrics
                            #                      Feature #4838: functions.metrics.get_namespace_metric.count
                            # This entire active_metric_id block was under-indented
                            # so indented properly while handling manage_inactive_metrics
                            # in the cluster context, which highlighted that this
                            # was adding all metrics due to the
                            # Add metrics reset to active added on 20230123
                            active_metric_id = None
                            try:
                                active_metric_id = analyzer_labelled_metrics_last_timeseries_timestamps[str(c_metric_id)]
                                if active_metric_id:
                                    active_labelled_metrics_with_id[c_metric_name] = c_metric_id
                            except:
                                pass

                            # @added 20230123 - Task #2732: Prometheus to Skyline
                            #                   Branch #4300: prometheus
                            # Add metrics reset to active
                            if active_metric_id:
                                if c_metric_id in metric_ids_to_reactive:
                                    active_labelled_metrics_with_id[c_metric_name] = c_metric_id
                                    active_labelled_ids_with_metrics[c_metric_id] = c_metric_name
                                # and add all metrics reported active by the database
                                active_labelled_metrics_with_id[c_metric_name] = c_metric_id
                                active_labelled_ids_with_metrics[c_metric_id] = c_metric_name
                    ...
                    # base_names can be looked up if they become active again in
                    # labelled_metrics.unique_metrics
                    if labelled_metrics_active_ids:
                        labelled_metrics_active_ids_set = set(labelled_metrics_active_ids)
                        active_labelled_ids_with_metrics_set = set(list(active_labelled_ids_with_metrics.keys()))
                        set_difference = labelled_metrics_active_ids_set.difference(active_labelled_ids_with_metrics_set)
                        if set_difference:
                            readding_labelled_metrics_errors = []
                            readded_active_again_labelled_metrics = 0
                            for metric_id in list(set_difference):
                                metric_name = None
                                try:
                    ...
                    if active_labelled_metrics_with_id:
                        ...
                        for metric in list(active_labelled_metrics_with_id.keys()):
                            metric_id = active_labelled_metrics_with_id[metric]
                            active_labelled_ids_with_metrics[metric_id] = metric
                        ...
                        # To handle labelled_metrics, allow for the request to pass a metric
                        # to determine if it is a derivative_metric or not
                        logger.info('metrics_manager :: determining list of derivative_labelled_metrics')
                        ...
                        for active_labelled_metric in active_labelled_metrics_with_id:
                            metric_type = None
                            try:
                                metric_type = skyline_labelled_metrics_id_types[str(active_labelled_metrics_with_id[active_labelled_metric])]
                            except:
                                continue
                            if str(metric_type) == '1':
                                derivative_labelled_metrics.append(active_labelled_metric)
                        logger.info('metrics_manager :: determined list of %s derivative_labelled_metrics from the %s active labelled_metrics' % (
                            str(len(derivative_labelled_metrics)), str(len(active_labelled_metrics_with_id))))

                        # @added 20230519 - metric_type.longterm_expire
                        # Determine labelled_metrics longterm monotonicity
                        logger.info('metrics_manager :: managing labelled_metrics longterm metric_type')
                        updated_types = 0
                        checked_ids = 0
                        try:
                            checked_ids, updated_types = manage_labelled_metrics_longterm_metric_type(self, RUN_EVERY, active_labelled_metrics_with_id)
                        except Exception as err:
                            logger.error('error :: labelled_metrics_spin_process :: manage_labelled_metrics_longterm_metric_type failed - %s' % err)
                        logger.info('metrics_manager :: checked: %s and updated %s labelled_metrics longterm metric_types' % (
                            str(checked_ids), str(updated_types)))

                    else:
                        logger.info('metrics_manager :: no entries in active_labelled_metrics_with_id deleting Redis hashes')
                        try:
                            self.redis_conn.delete('metrics_manager.active_labelled_ids_with_metric')
                            self.redis_conn.delete('aet.metrics_manager.active_labelled_ids_with_metric')
                            logger.info('metrics_manager :: deleted the metrics_manager.active_labelled_ids_with_metric and aet.metrics_manager.active_labelled_ids_with_metric Redis hashes')
                        except Exception as err:
                            logger.error(traceback.format_exc())
                            logger.error('error :: metrics_manager :: failed to delete metrics_manager.active_labelled_ids_with_metric and aet.metrics_manager.active_labelled_ids_with_metric Redis hashes - %s' % err)
                        logger.info('metrics_manager :: no entries in active_labelled_metrics_with_id deleting Redis hashes')
                        try:
                            self.redis_conn.delete('metrics_manager.active_labelled_metrics_with_id')
                            self.redis_conn.delete('aet.metrics_manager.active_labelled_metrics_with_id')
                            logger.info('metrics_manager :: deleted the metrics_manager.active_labelled_metrics_with_id and aet.metrics_manager.active_labelled_metrics_with_id Redis hashes')
                        except Exception as err:
                            logger.error(traceback.format_exc())
                            logger.error('error :: metrics_manager :: failed to delete metrics_manager.active_labelled_metrics_with_id and aet.metrics_manager.active_labelled_metrics_with_id Redis hashes - %s' % err)

                logger.info('metrics_manager :: metric_management_process :: memory usage after metric names and ids management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                for labelled_metric_base_name in list(active_labelled_metrics_with_id.keys()):
                    try:
                        ...
                        if pattern_match:
                            ...
                            if settings.DO_NOT_SKIP_SKYLINE_FEEDBACK_NAMESPACES:
                                pattern_match, matched_by = matched_or_regexed_in_list(skyline_app, labelled_metric_base_name, settings.DO_NOT_SKIP_SKYLINE_FEEDBACK_NAMESPACES, False)
                                if pattern_match:
                                    feedback_metric = False
                            if feedback_metric:
                                metric_id = active_labelled_metrics_with_id[labelled_metric_base_name]
                                feedback_labelled_metric_ids.append(metric_id)
                    except Exception as err:
                ...
                if mute_alerts_on_dict:
                    for metric in list(mute_alerts_on_dict.keys()):
                        if metric.startswith('labelled_metrics.'):
                            metric_id_str = metric.split('.')[-1]
                            feedback_labelled_metric_ids.append(int(metric_id_str))
                ...
                #                   Branch #4300: prometheus
                if active_labelled_ids_with_metrics:
                    logger.info('metrics_manager :: running manage_smtp_alerter_labelled_metrics passing %s ids_with_metric_names' % str(len(ids_with_metric_names)))
                    smtp_alert_labelled_metrics = []
                    try:
                        smtp_alert_labelled_metrics = manage_smtp_alerter_labelled_metrics(self, ids_with_metric_names)
                    except Exception as err:
                        logger.error(traceback.format_exc())
                        logger.error('error :: metrics_manager :: manage_smtp_alerter_labelled_metrics failed - %s' % (
                            err))
                        smtp_alert_labelled_metrics = []
                    logger.info('metrics_manager :: ran manage_smtp_alerter_labelled_metrics %s smtp alert enabled metrics determined' % str(len(smtp_alert_labelled_metrics)))
                    del smtp_alert_labelled_metrics
                ...
                #                   Branch #4300: prometheus
                # Run redistimeseries_roomba
                logger.info('metrics_manager :: running redistimeseries_roomba passing %s active_labelled_ids_with_metrics' % str(len(active_labelled_ids_with_metrics)))
                if active_labelled_ids_with_metrics:
                    sample_data_key = list(active_labelled_ids_with_metrics.keys())[0]
                    sample_data = active_labelled_ids_with_metrics[sample_data_key]
                    sample_dict = {sample_data_key: sample_data}
                    logger.debug('debug :: metrics_manager :: active_labelled_ids_with_metrics sample: %s' % str(sample_dict))
                ...
                try:
                    ...
                    logger.error('error :: metrics_manager :: redistimeseries_roomba failed - %s' % (
                        err))
                logger.info('metrics_manager :: ran redistimeseries_roomba and %s labelled_metrics were removed' % str(len(roombaed_redistimeseries_metrics)))
                try:
                    del roombaed_redistimeseries_metrics
                except:
                    pass
                del ids_with_labelled_metric_names
                del active_labelled_ids_with_metrics
                logger.info('metrics_manager :: metric_management_process :: memory usage after redistimeseries_roomba management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))

                # @added 20220727 - Task #2732: Prometheus to Skyline
                #                   Branch #4300: prometheus
                # Prune the skyline.expose.prometheus.metrics
                if EXPOSE_PROMETHEUS_METRICS:
                    try:
                        pruned_count = prune_expose_prometheus_metrics(self)
                    except Exception as err:
                        logger.error(traceback.format_exc())
                        logger.error('error :: metrics_manager :: prune_expose_prometheus_metrics failed - %s' % (
                            err))
                    logger.info('metrics_manager :: %s keys older than 5 minutes were pruned from skyline.expose.prometheus.metrics' % (
                        str(pruned_count)))

                # @added 20221125 - Feature #4734: mirage_vortex
                # Prune the mirage.vortex and flux.vortex hash keys
                if VORTEX_ENABLED:
                    logger.info('metrics_manager :: purging any old keys in vortex hash keys')
                    c_time = int(time())
                    vortex_purge_times = {'flux.vortex': 300, 'mirage.vortex': 3600}
                    for vortex_hash_key in ['flux.vortex', 'mirage.vortex']:
                        vortex_keys = []
                        try:
                            vortex_keys = self.redis_conn_decoded.hkeys(vortex_hash_key)
                        except Exception as err:
                            logger.error('error :: metrics_manager :: failed to hkeys on %s - %s' % (
                                vortex_hash_key, str(err)))
                        v_errors = []
                        remove_v_keys = []
                        for v_key in vortex_keys:
                            try:
                                v_ts = int(v_key.split('.', 1)[0])
                            except Exception as err:
                                v_errors.append([v_key, 'failed to determine timestamp', err])
                                continue
                        ...
                        if remove_v_keys:
                            ...
                            try:
                                self.redis_conn_decoded.hdel(vortex_hash_key, *set(remove_v_keys))
                            except Exception as err:
                                logger.error('error :: metrics_manager :: failed to hdel keys from %s - %s' % (
                                    vortex_hash_key, str(err)))
                    # @added 20221130 - Feature #4734: mirage_vortex
                    try:
                        vortex_saved_data_purged_count = purge_vortex_saved_data(self, VORTEX_SAVE_RESULTS_FOR, settings.SKYLINE_DIR)
                    except Exception as err:
                        logger.error('error :: metrics_manager :: purge_vortex_saved_data failed - %s' % (
                            str(err)))
                ...
                # @modified 20230201 - Feature #4792: functions.metrics_manager.manage_inactive_metrics
                # Only run if there are always_derivative_metrics
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after derivative management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                logger.info('metrics_manager :: metric_management_process :: memory usage after batch_processing management - %s' % (
                    str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                ...
                try:
                    send_graphite_metric(self, skyline_app, send_metric_name, str(spin_end))
            ...
            logger.error(traceback.format_exc())
            logger.error('error :: metrics_manager :: with memray failed - %s' % err)
    ...
    def run(self):
        ...
        while 1:
            ...
            logger.info('metrics_manager :: memory usage at start of run - %s' % (
                str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
            ...
            logger.info('metrics_manager :: memory usage after start of run - %s' % (
                str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
            ...
            logger.info('metrics_manager :: memory usage after end of run - %s' % (
                str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss)))
                        ...
                        while sync_time < sync_until:
                            if last_sync_cluster_time:
                                ...
                                if sync_sleep_for > 0:
                                    ...
                                    # @modified 202200512 - Feature #4324: flux - reload external_settings
                                    #                    Feature #4376: webapp - update_external_settings
                                    # Break out of sync_cluster_files if reload_flux is
                                    # required
                                    # sleep(sync_sleep_for)

                                    # @added 202200512 - Feature #4324: flux - reload external_settings
                                    #                    Feature #4376: webapp - update_external_settings
                                    # Break out of sync_cluster_files if reload_flux is
                                    # required
                                    sleep_start = int(time())
                                    update_error_logged = False
                                    while int(time()) < (sleep_start + sync_sleep_for):
                                        external_settings_updated = None
                                        try:
                                            external_settings_updated = self.redis_conn_decoded.get('skyline.external_settings.update.metrics_manager')
                                        except Exception as err:
