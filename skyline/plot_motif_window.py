...
def plot_motif_window(
    current_skyline_app, metric, metric_timestamp, fp_id, full_duration,
        ...
        fp_timeseries, matched_timeseries, output_file, strip_prefix=False):
    """
    Creates a png graph image using the features profile time seires data and
    the training data data, if it exists otherwise if is fetched from Graphite.
    ...
    if os.path.isfile(output_file):
        current_logger.info('plot_motif_window - graph image already exists - %s' % output_file)
    ...
    try:
        ...
        fig.clf()
        plt.close(fig)
        current_logger.info('plot_motif_window - created graph image - %s' % output_file)
    except:
        current_logger.error(traceback.format_exc())
        current_logger.error('error :: plot_motif_window :: failed to create %s' % output_file)
        return (False, None)
