...
def moving_sum_and_value_decrease(current_skyline_app, parent_pid, timeseries, algorithm_parameters):
    """
    ...
    algorithm_name = 'moving_sum_and_value_decrease'
    ...
    less_than_moving_sum_average = 3
    try:
        less_than_moving_sum_average = algorithm_parameters['less_than_moving_sum_average']
    except KeyError:
        less_than_moving_sum_average = 3
    except Exception as e:
        record_algorithm_error(current_skyline_app, parent_pid, algorithm_name, traceback.format_exc())
    ...
    run_longest_zero_streak = False
    try:
        run_longest_zero_streak = algorithm_parameters['run_longest_zero_streak']
    except KeyError:
        run_longest_zero_streak = False
    ...
    try:
        ...
        try:
            ...
            for index, timestamp in enumerate(y_np):
                moving_sum_ts.append([int(timestamp), float(moving_sum_values[index])])
            ...
            for index, item in enumerate(timeseries):
                ...
                moving_sum_average = sum(last_datapoints) / len(last_datapoints)
                percent_different = get_percent_different(moving_sum_average, item[1], True)
                if percent_different:
                    if percent_different < 0:
                        new_pdiff = percent_different * -1
                        percent_different = new_pdiff
#                    if percent_different > percentage_difference and item[1] < (moving_sum_average * 3):
                    if percent_different > percentage_difference and item[1] < (moving_sum_average / less_than_moving_sum_average):
                        if item[1] > 0 and results_timeseries[-1][1][1] > 0:
                            anomaly_score = 0.7
                            if debug_logging:
                                if item[0] == last_timestamp:
                                    # anomaly_trigger = 'moving_sum less_than 3 * moving_sum_average'
                                    anomaly_trigger = 'moving_sum moving_sum_average is less_than moving_sum_average / %s' % str(less_than_moving_sum_average)
                                    current_logger.debug('debug :: %s :: percent_different (%s) > percentage_difference and item[1] (%s) < (%s * moving_sum_average) (%s) for %s' % (
                                        algorithm_name, str(percent_different),
                                        str(item[1]), str(moving_sum_average),
                                        str(less_than_moving_sum_average),
                                        str(item[0])))
                else:
                    percent_different = 0
                    results_timeseries.append([index, item, moving_sum_average, anomaly_score, 0, original_timeseries_use_last[index]])
                    continue
                last_moving_sum_average = results_timeseries[-1][2]
                constant_decrease = False
                decreases = []
                window_indices = list(range(2, (window - 1)))
                if moving_sum_average < last_moving_sum_average:
                    last_value = last_moving_sum_average
                    # for i in [2, 3, 4, 5, 6]:
                    for i in window_indices:
                        ...
                        if last_value < new_last_value and lm_percent_different > percentage_difference:
                            ...
                            decreases.append(last_value)
                        else:
                            constant_decrease = False
                            # break
                # if constant_decrease or len(decreases) >= (len(window_indices) - 1):
                if constant_decrease or len(decreases) >= (len(window_indices) - 1):
                    anomaly_score = 0.8
                    if debug_logging:
                        if item[0] == last_timestamp:
                            anomaly_trigger = 'moving_sum_average of moving_sum has decreased %s times' % str(len(decreases))
                            current_logger.debug('debug :: %s :: moving_sum_average of moving_sum has decreased %s times, anomaly_score: %s' % (
                                algorithm_name, str(len(decreases)), str(anomaly_score)))

                # Big increase
                if percent_different:
                    if percent_different > percentage_difference and original_timeseries_use_last[index][1] > (moving_sum_average * 3):
                        anomaly_score = 0.7
                        if debug_logging:
                            if item[0] == last_timestamp:
                                anomaly_trigger = 'significant increase, datapoint > 3 * moving_sum_average of moving_sum'
                                current_logger.debug('debug :: %s :: large increase percent_different (%s) > percentage_difference and datapoint (%s) > (moving_sum_average * 3) (%s) for %s' % (
                                    algorithm_name, str(percent_different),
                                    str(original_timeseries_use_last[index][1]), str((moving_sum_average * 3)),
                                    str(item[0])))
                ...
                if anomaly_score > 0:
                    ...
                    score = 1
                    results['anomalies'][ts] = {'value': original_timeseries_use_last[index][1], 'index': index, 'score': score}
#                    if realtime_analysis:
#                        if item[0] == last_timestamp:
#                            anomalous = True
                results_timeseries.append([index, item, moving_sum_average, anomaly_score, percent_different, original_timeseries_use_last[index]])
                # use correct value
            ...
            if debug_logging:
                current_logger.debug('debug :: %s :: %s after moving_sum_decrease, anomalies: %s, running longest_zero_streak: %s' % (
                    algorithm_name, base_name, str(len(results['anomalies'])),
                    str(run_longest_zero_streak)))

            # longest zero streak
            if run_longest_zero_streak:
                min_zero_streak = 8
                max_zero_streak = 10
