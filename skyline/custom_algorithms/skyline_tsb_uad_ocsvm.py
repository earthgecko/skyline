...
from custom_algorithm_sources.TSB_UAD.ocsvm import OCSVM as tsb_uad_ocsvm
from custom_algorithm_sources.TSB_UAD.utils import find_length
from custom_algorithm_sources.TSB_UAD.feature import Window
...
# @added 20251018 - Feature #5656: custom_algorithm - skyline_tsb_uad_ocsvm
def skyline_tsb_uad_ocsvm(current_skyline_app, parent_pid, timeseries, algorithm_parameters):
    """

    The skyline_tsb_uad_ocsvm algorithm uses a version of OCSVM that is based on
    a version implemented in the TSB-UAD library.

    It is convience wrapper to run TSB_UAD.models.ocsvm
    ...
    tornado method as the algorithm load time and initialisation takes 2.5 seconds
    ...
    :param algorithm_parameters: a dictionary of the required parameters for
        the skyline_tsb_uad_ocsvm custom_algorithm.  These are documented in the
        custom_algorithm code itself so they will not be redocumented here.
        Refer to the algorithm_parameters and example usage in the skyline_tsb_uad_ocsvm
        docstrings
        https://earthgecko-skyline.readthedocs.io/en/latest/skyline.custom_algorithms.html#module-custom_algorithms.skyline_tsb_uad_ocsvm
        ...
        - ``'nu'`` (float): The nu value.  The default tsb_uad OCSVM nu value is
            0.05.  Testing showed that there is very little difference between
            0.05 and 0.09 therefore having 5 % outliers instead of 1 % outliers
            is not much different, but 0.05 is probably preferable to detect
            more subtle changes were 0.09 may miss some.  Default is ``0.05``.
    ...
    algorithm_name = 'skyline_tsb_uad_ocsvm'
    ...
    # TSB_UAD OCSVM algorithm parameters
    # The default tsb_uad OCSVM nu value is 0.05.  Testing showed that there is
    # very little difference between 0.05 and 0.09 therefore having 5 % outliers
    # instead of 1 % outliers is not much different, but 0.05 is probably
    # preferable to detect more subtle changes were 0.09 may miss some.
    nu = 0.05
    try:
    ...
    try:
        ...
        # If the series is constant OCSVM and statsmodels/tsa/stattools.acf will
        ...
        if tornado_url and tornado_api_key:
            if debug_logging:
                current_logger.debug('debug :: %s :: using tornado tsb_uad_ocsvm' % (
                    func_name))
            ...
            if debug_logging:
                current_logger.debug('debug :: %s :: tornado tsb_uad_ocsvm took %.6f seconds' % (
        ...
        if not tornado_url and not tornado_api_key:
            try:
                ...
                #                      Feature #5656: custom_algorithm - skyline_tsb_uad_ocsvm
                #data = np.array([value for _, value in timeseries])
                data = np_values
                slidingWindow = find_length(data)
                # Prepare data for semisupervised method. 
                # Here, the training ratio = 0.1
                #data_train = data[:int(0.1*len(data))]
                train_length = int(0.1*len(data))
                if train_length < slidingWindow:
                    train_length = int(slidingWindow)
                ...
                # Using 6-sigma as the threshold unless > 1 then use the 75th
                # percentile
                ...
                for value in score.tolist():
                    if str(value) in ['NaN','nan','Nan']:
                        ...
                        try:
                            ...
                            value = 0
                    ocsvm_scores.append(value)

                for index, item in enumerate(timeseries):
                    if ocsvm_scores[index] > threshold:
                        anomalies[item[0]] = {'index': index, 'value': item[1], 'score': 1, 'anomalyScore': ocsvm_scores[index]}
                        ocsvm_anomalies.append(1)
                    else:
                        ocsvm_anomalies.append(0)
                results = {'anomalies': anomalies}
                if debug_logging:
                    current_logger.info('%s :: len(anomalies): %s' % (
                        func_name, str(len(anomalies))))

                anomaly_sum = sum(ocsvm_anomalies[-anomaly_window:])
                if debug_logging:
        ...
        if return_results:
            results = {
                'anomalous': anomalous,
                'anomalies': anomalies,
                'anomalyScore_list': ocsvm_anomalies,
                'scores': ocsvm_scores,
            }
