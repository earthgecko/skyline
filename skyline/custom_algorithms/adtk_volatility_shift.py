# @added 20210305 - Feature #3976: custom_algorithm - adtk_volatility_shift
...
def adtk_volatility_shift(current_skyline_app, parent_pid, timeseries, algorithm_parameters):
    ...
    timestamp will be added to the ``analyzer.last.adtk_volatility_shift`` Redis hash
    ...
    The Skyline CUSTOM_ALGORITHMS implementation of the adtk VolatilityShiftAD
    ...
    algorithm_name = 'adtk_volatility_shift'
    ...
    # Allow the VolatilityShiftAD window parameter to be passed in the
    # algorithm_parameters
    window = 5
    try:
    ...
    # Allow the VolatilityShiftAD c parameter to be passed in the
    # algorithm_parameters
    c = 9.0
    try:
    ...
    # Due to the load and runtime of VolatilityShiftAD it is only run in analyzer
    # periodically
    ...
    try:
        ...
        # of Skyline's use of the VolatilityShiftAD algorithm this means that:
        ...
        # @added 20210305 - Feature #3970: custom_algorithm - adtk_volatility_shift
        #                   Task #3664:: POC with adtk
        # With mirage also do not run VolatilityShiftAD on sparsely populated data
        ...
        if not timeseries:
            if debug_logging:
                current_logger.debug('debug :: %s :: VolatilityShiftAD not run as no data' % (
                    algorithm_name))
            anomalies = []
            if return_anomalies:
                return (anomalous, anomalyScore, anomalies)
        ...
        try:
            ...
            anomaly_df = volatility_shift_ad.fit_detect(s)
        ...
        if debug_logging:
            current_logger.debug('debug :: %s :: VolatilityShiftAD took %.6f seconds' % (
