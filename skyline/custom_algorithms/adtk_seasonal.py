...
def adtk_seasonal(current_skyline_app, parent_pid, timeseries, algorithm_parameters):
    """
    ...
    timestamp will be added to the ``analyzer.last.adtk_seasonal`` Redis hash
    ...
    The Skyline CUSTOM_ALGORITHMS implementation of the adtk SeasonalAD
    ...
    # window is not used in the SeasonalAD algorithm but it
    # ys used to determine if anomalies occur in the
    # Skyline adtk_seasonal window context
    try:
        window = algorithm_parameters['window']
    except:
        window = 5

    # Allow the SeasonalAD c parameter to be passed in the
    # algorithm_parameters
    c = 9.0
    try:
    ...
    # Due to the load and runtime of SeasonalAD it is only run in analyzer
    # periodically
    ...
    try:
        ...
        # of Skyline's use of the SeasonalAD algorithm this means that:
        ...
        #                   Task #3664:: POC with adtk
        # With mirage also do not run SeasonalAD on sparsely populated data
        ...
        if not timeseries:
            if debug_logging:
                current_logger.debug('debug :: %s :: SeasonalAD not run as no data' % (
                    algorithm_name))
            anomalies = []
            if return_anomalies:
                return (anomalous, anomalyScore, anomalies)
        ...
        try:
			...
			# Unlike other adtk_ custom algorithms seansonal
            # is wrapped in try except as if seasonal finds
            # no seasonal trends or recognised frequeny it
            # throws an exception
            try:
                seasonal_ad = SeasonalAD(c=c, side=side)
                anomaly_df = seasonal_ad.fit_detect(s)
        ...
        if debug_logging:
            current_logger.debug('debug :: %s :: SeasonalAD took %.6f seconds' % (
