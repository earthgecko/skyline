...
time series.  These are metrics that are integral (an incrementing count) in
...
time series to their derivative products through most of the Skyline
...
strictly increasing monotonic behaviour at times.  For example take some stock
...
:mod:`settings.FULL_DURATION` seconds of time series data, while it was
...
profiling and learning that can be done on this type of time series data.
...
Skyline's time series comparison and analysis methods.
