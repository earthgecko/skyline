...
def feed_data(sock, data, newNow):
   tuples = ([])
   tuples.append(('system.loadavg_1min', (newNow,data[0])))
   tuples.append(('system.loadavg_5min', (newNow,data[1])))
   tuples.append(('system.loadavg_15min', (newNow,data[2])))
...
def run_foo(sock):
   now = int(time.time()) # - (60 * 60 * 24 * 1)
   ntime = now - (60 * 60 * 24 * 100)
   print(ntime)
   count = 0
   while ntime < now:
       count = count + 1
       if  ntime > now - (60 * 60 * 24 * 2):
           feed_data(sock, [count, 100, count], ntime)
           print(ntime)
           print("sending 100")
       else:
           feed_data(sock, [0, 0, 0], ntime)
#          print(ntime)
       ntime = ntime +  60
   print(count)
